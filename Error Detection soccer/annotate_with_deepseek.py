...
# 上一轮 name/surname/birthplace 漏检严重，因此不再设置 context-only 列。
...
12. team-context evidence such as team+season -> stadium/city/manager is useful but not absolute. A football team may change manager, stadium, or host city depending on season or data source.
...
def get_sample_summary_context(sample_obj: Dict[str, Any]) -> Dict[str, Any]:
    """从 sampled JSONL 自动构造 summary_context。

    兼容两种输入：
    1. 新版 sampled JSONL 中已经有 summary_context；
    2. 只有 detail_context / 原始 detail obj / sampling_info 的 JSONL。

    这样可以避免标注输出 CSV 中训练字段大量为空。
    """
    summary_ctx = dict(sample_obj.get("summary_context", {}) or {})

    # sampling_info 通常由 cluster_and_sample_candidates.py 写入
    sampling_info = sample_obj.get("sampling_info", {}) or {}
    for k in [
        "bucket_id", "cluster_id", "cluster_size", "dist_to_center", "sample_role",
        "soccer_signal_bucket", "soccer_attribution_bucket",
        "rarity_bucket", "pattern_bucket", "neighbor_bucket",
        "target_is_primary_column", "target_is_context_only_column",
        "main_rule_type",
    ]:
        if k in sampling_info and k not in summary_ctx:
            summary_ctx[k] = sampling_info.get(k)

    # detail_context 可能包了一层；也可能 sample_obj 本身就是 detail object
    detail = sample_obj.get("detail_context", None)
    ...
    base_defaults.update({
        "year_value": soccer_value_features.get("year_value"),
        "canonical_position": soccer_value_features.get("canonical_position"),
        "domain_valid": soccer_value_features.get("domain_valid"),
        "soccer_value_bucket": soccer_value_features.get("soccer_value_bucket"),
    })

    # soccer attribution features
    for k, v in soccer_attr.items():
        base_defaults[k] = v

    # 如果没有 soccer_attr，则根据 column/rule 做最低限度补齐
    col = str(sample_obj.get("column", detail.get("column", "")))
    rule = str(base_defaults.get("main_rule_type") or summary_ctx.get("main_rule_type") or "")

    base_defaults.setdefault("target_is_primary_column", 1 if col in PRIMARY_TARGET_COLUMNS else 0)
    base_defaults.setdefault("target_is_context_only_column", 1 if col in CONTEXT_ONLY_COLUMNS else 0)
    base_defaults.setdefault("birthyear_value_invalid", 1 if rule == "birthyear_numeric_range" else 0)
    base_defaults.setdefault("season_value_invalid", 1 if rule == "season_numeric_range" else 0)
    base_defaults.setdefault("position_value_invalid", 1 if rule == "position_domain" else 0)
    base_defaults.setdefault("position_needs_canonicalization", 0)
    base_defaults.setdefault("birthyear_season_age_conflict", 1 if rule == "birthyear_season_age_consistency" else 0)
    base_defaults.setdefault("team_context_conflict", 1 if rule in SOCCER_CONTEXT_RULE_TYPES else 0)
    base_defaults.setdefault("format_rule_signal", 1 if rule in SOCCER_FORMAT_RULE_TYPES else 0)
    base_defaults.setdefault("fd_like_signal", 1 if rule in SOCCER_FD_RULE_TYPES else 0)
    base_defaults.setdefault("evidence_only_fd_count", 1 if rule in SOCCER_EVIDENCE_ONLY_FD_RULE_TYPES else 0)

    attr = base_defaults.get("soccer_attribution_bucket") or summary_ctx.get("soccer_attribution_bucket")
    if not attr or str(attr) in {"missing", "unknown", "unknown_attribution", "nan", "None"}:
        if base_defaults.get("birthyear_value_invalid"):
            attr = "birthyear_invalid"
        elif base_defaults.get("season_value_invalid"):
            attr = "season_invalid"
        elif base_defaults.get("position_value_invalid"):
            attr = "position_invalid"
        elif base_defaults.get("position_needs_canonicalization"):
            attr = "position_canonicalization"
        elif base_defaults.get("birthyear_season_age_conflict"):
            attr = "birthyear_season_age_conflict"
        elif base_defaults.get("team_context_conflict"):
            attr = "team_context_conflict"
        elif base_defaults.get("format_rule_signal"):
            attr = "format_pattern_conflict"
        elif base_defaults.get("fd_like_signal"):
            attr = "fd_like_conflict"
        elif col in PRIMARY_TARGET_COLUMNS:
            attr = "primary_target_other"
        else:
            attr = "unknown_attribution"
    base_defaults["soccer_attribution_bucket"] = attr

    # merge：已有 summary_context 优先，但空值用 base_defaults 补齐
    for k, v in base_defaults.items():
        if k not in summary_ctx or summary_ctx.get(k) is None or str(summary_ctx.get(k)) == "nan":
            summary_ctx[k] = v

    # v2 强制纠偏
    summary_ctx["target_is_primary_column"] = 1 if col in PRIMARY_TARGET_COLUMNS else as_int01(summary_ctx.get("target_is_primary_column"))
    summary_ctx["target_is_context_only_column"] = 1 if col in CONTEXT_ONLY_COLUMNS else 0
...
def build_summary_prefix(sample_obj: Dict[str, Any]) -> str:
    ...
    """
    summary_ctx = get_sample_summary_context(sample_obj)
    ...
    if main_rule == "birthyear_season_age_consistency":
        ...
        lines.append("")
        lines.append("Important attribution hint:")
        lines.append("- In soccer v2 this column is a target column. Missing or clear format evidence is strong; rarity alone is weak.")
...
def build_user_prompt(sample_obj: Dict[str, Any]) -> str:
    ...
    """
    detail_ctx = sample_obj.get("detail_context", None)
    if not isinstance(detail_ctx, dict):
        detail_ctx = sample_obj
...
def apply_soccer_label_sanity(sample_obj: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out = dict(result)
    summary_ctx = get_sample_summary_context(sample_obj) or {}
    col = str(sample_obj.get("column", ""))
    ...
    format_signal = as_int01(summary_ctx.get("format_rule_signal", 0)) == 1
    schema_rule_count = safe_float(summary_ctx.get("schema_rule_count", 0), 0)
    soccer_format_count = safe_float(summary_ctx.get("soccer_format_rule_count", 0), 0)
    main_rule_is_missing = rule in {"not_null", "high_risk_missing"}

    # soccer v2: name/surname/birthplace 是 target columns。缺失或明确格式错误应当作为强证据。
    if col in {"name", "surname", "birthplace", "manager"} and (main_rule_is_missing or format_signal or soccer_format_count > 0) and out.get("label") == "correct":
        out["label"] = "error"
        out["is_error"] = True
        out["confidence"] = max(float(out.get("confidence", 0.5)), 0.82 if main_rule_is_missing else 0.78)
        out["error_type"] = "missing_value_error" if main_rule_is_missing else "format_error"
        ev = set(out.get("evidence_used", []))
        ev.update({"format_constraint", "column_context"} if not main_rule_is_missing else {"row_context", "column_context"})
        out["evidence_used"] = sorted(ev)
        out["reason_short"] = out.get("reason_short") or f"{col} has direct missing or format evidence."
        out["reason_detailed"] = (
            (out.get("reason_detailed") or "")
            + f" Soccer v2 treats {col} as a target column; direct missing/format evidence is strong."
        ).strip()
    ...
    if out.get("label") == "error":
        direct_strong = (
            ...
            or rule in {"not_null", "high_risk_missing"}
        )

        if col in {"name", "surname", "birthplace", "team", "city", "stadium", "manager"} and not direct_strong:
            out["confidence"] = min(float(out.get("confidence", 0.5)), 0.70)
            out["needs_human_review"] = True
            out["reason_detailed"] = (
                (out.get("reason_detailed") or "")
                + " Long-tail entity column without direct missing/domain/format evidence; downgrade confidence."
            ).strip()
...
def flatten_output_row(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "evidence_only_fd_count": summary_ctx.get("evidence_only_fd_count"),
...
def build_output_object(sample: Dict[str, Any], label_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "bucket_id": sample.get("bucket_id", sampling_info.get("bucket_id")),
        "cluster_id": sample.get("cluster_id", sampling_info.get("cluster_id")),
        "sample_role": sample.get("sample_role", sampling_info.get("sample_role")),
        "dist_to_center": sample.get("dist_to_center", sampling_info.get("dist_to_center")),
        "signal_priority": sample.get("signal_priority", sampling_info.get("soccer_signal_bucket")),
        "summary_context": get_sample_summary_context(sample),
