...
# 是否在 similar rows 中保留完整 row_values。
# 为了保持你原有 JSONL 输出结构，默认 True。
INCLUDE_SIMILAR_ROW_VALUES = True

# 相似度 batch 大小。v2 中只对候选行计算相似行，不再对 20 万全表行都建 neighbor index。
SIMILARITY_BATCH_SIZE = 2048
...
SOCCER_CATEGORICAL_COLUMNS = {"position", "team", "city", "stadium"}

# ============================================================
# Soccer v2 关键修改
# ============================================================
# 上一轮评估中 name / surname / birthplace 大量真实错误没有进入候选。
# 因此上下文构造阶段也必须同步规则池 v2 和 shrink v2：
# 所有 soccer 列都作为 primary target；不再把 name/surname 视为 context-only。
...
SOCCER_CONTEXT_RULE_TYPES = {
    "team_context_dominant", "team_season_manager_consistency_hint", "city_stadium_consistency_hint",
    "dominant_value_by_context", "dominant_value_by_context_pair",
}

# v2: soft-FD 只作为 evidence-only，不再作为候选/强特征信号。
# 如果旧候选文件里还残留 soft_functional_dependency，这里不会把它计入 fd_like_signal。
...
_YEAR_CACHE: Dict[Any, Optional[int]] = {}
_VALUE_FEATURE_CACHE: Dict[Tuple[str, Any], Dict[str, Any]] = {}
...
def parse_year(value) -> Optional[int]:
    if value in _YEAR_CACHE:
        return _YEAR_CACHE[value]
...
def is_id_like_column(non_null: pd.Series, unique_ratio: float, numeric_ratio: float, avg_len: float) -> bool:
    return len(non_null) > 0 and (
        (unique_ratio >= ID_LIKE_UNIQUE_RATIO_THRESHOLD and avg_len <= CODE_LIKE_AVG_LEN_THRESHOLD)
        or (unique_ratio >= ID_LIKE_UNIQUE_RATIO_THRESHOLD and numeric_ratio >= NUMERIC_RATIO_THRESHOLD)
    )
...
def is_code_like_column(
    ...
    if (
        avg_len <= CODE_LIKE_AVG_LEN_THRESHOLD
        and unique_ratio > CATEGORICAL_UNIQUE_RATIO_THRESHOLD
        and top_patterns
        and top_patterns[0]["ratio"] >= 0.7
    ):
        return True
...
def analyze_columns(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    for col in df.columns:
        ...
        elif col in SOCCER_CATEGORICAL_COLUMNS:
            detected_type = "categorical"
        elif col in {"name", "surname", "manager", "birthplace"}:
            detected_type = "text" if unique_count > CATEGORICAL_UNIQUE_COUNT_THRESHOLD else "categorical"
...
def summarize_row_context(df: pd.DataFrame, row_id: int, col_profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    semantic_type_counts = Counter(
        col_profiles[col]["semantic_type"]
        for col, v in row.items()
        if v is not None
    )

    row_patterns = {
        col: detect_basic_pattern(v) if v is not None else "NULL"
        for col, v in row.items()
    }
    ...
    return {
        ...
        "soccer_profile_bundle": build_soccer_profile_bundle(row),
    }
...
def top_values_for_group(sub: pd.DataFrame, target: str) -> Dict[str, Any]:
    ...
    return {
        "top_values": [{"value": k, "count": v} for k, v in cnt.most_common(PROFILE_TOPK_VALUES)],
        "group_size": int(len(sub)),
    }


def precompute_soccer_group_distributions(df: pd.DataFrame):
...
def build_soccer_value_features(col: str, value: Any) -> Dict[str, Any]:
    ...
    if col == "birthyear" and year_value is not None:
        bucket = (
            "older_generation"
            if year_value < 1970
            else "typical_player_birthyear"
            if year_value <= 1995
            else "young_generation"
        )
...
def summarize_column_context(col, value, col_profiles):
    ...
    return {
        ...
        "value_frequency_rank": prof["value_rank_map"].get(value),
        ...
        "soccer_value_features": build_soccer_value_features(col, value),
    }
...
def summarize_conflict_info(candidate, col_profiles):
    ...
    fd_like = sum(rule_type_counter.get(x, 0) for x in SOCCER_FD_RULE_TYPES)
    evidence_only_fd = sum(rule_type_counter.get(x, 0) for x in SOCCER_EVIDENCE_ONLY_FD_RULE_TYPES)
    context_rule = sum(rule_type_counter.get(x, 0) for x in SOCCER_CONTEXT_RULE_TYPES)
    ...
    soccer_numeric = (
        rule_type_counter.get("birthyear_numeric_range", 0)
        + rule_type_counter.get("season_numeric_range", 0)
    )
    soccer_cons = (
    ...
    pattern_rule = rule_type_counter.get("pattern", 0) + rule_type_counter.get("rare_pattern", 0) + soccer_format
    schema_rule = (
    ...
    return {
        ...
        # v2: fd_like_count 只统计 functional_dependency，不再统计 soft_functional_dependency。
        "fd_like_count": fd_like,
        "evidence_only_fd_count": evidence_only_fd,
        ...
        "soccer_domain_rule_count": soccer_domain,
        "soccer_format_rule_count": soccer_format,
        "soccer_numeric_rule_count": soccer_numeric,
        "soccer_consistency_rule_count": soccer_cons,

        # 兼容旧 adult 字段名
        "adult_domain_rule_count": soccer_domain,
        "adult_format_rule_count": soccer_format,
        "adult_consistency_rule_count": soccer_cons,
        "time_window_rule_count": 0,

        "expected_values_from_rules": expected,
        "alternative_values_from_column": alt_col,
...
def encode_cell_for_similarity(series: pd.Series, semantic_type: str) -> np.ndarray:
    ...
    if semantic_type in {"birth_year", "season_year", "numeric"}:
        nums = pd.to_numeric(
...
def build_similarity_matrix(df: pd.DataFrame, col_profiles: Dict[str, Dict[str, Any]]) -> np.ndarray:
    blocks = [
        encode_cell_for_similarity(df[col], col_profiles[col]["semantic_type"])
        for col in df.columns
    ]
...
def precompute_neighbor_indices(X: np.ndarray, top_k: int) -> Dict[int, List[Tuple[int, float]]]:
    """旧版本：给全表每一行都计算邻居。保留函数以兼容，但 main 中不再调用。"""
    n = X.shape[0]
    result = {}
    k = min(top_k + 1, n)
...
def precompute_neighbor_indices_for_candidate_rows(
    X: np.ndarray,
    candidate_row_ids: List[int],
    top_k: int
) -> Dict[int, List[Tuple[int, float]]]:
    """v2 加速版：只给候选行计算 top-k 相似行。

    输出结构和旧函数完全一致：
    {
        row_id: [(neighbor_row_id, similarity), ...]
    }
    """
    n = X.shape[0]
    result = {}
    ...
    candidate_row_ids = sorted(set(int(x) for x in candidate_row_ids if 0 <= int(x) < n))

    for start in range(0, len(candidate_row_ids), SIMILARITY_BATCH_SIZE):
        batch_row_ids = candidate_row_ids[start:start + SIMILARITY_BATCH_SIZE]
        if not batch_row_ids:
            continue

        batch_X = X[batch_row_ids]
        sims = batch_X @ X.T

        for local_i, row_id in enumerate(batch_row_ids):
            sims[local_i, row_id] = -np.inf
...
def summarize_similarity_info_fast(
    ...
    majority_ratio = (
        round(dist.most_common(1)[0][1] / len(target_values), 6)
        if dist and target_values
        else None
    )
...
# ============================================================
# 8. Soccer consistency / bayes / 文本
# ============================================================

def summarize_soccer_consistency_info_fast(
    df,
    row_id,
    col,
    value,
    ...
    row = df.iloc[row_id].to_dict()
    profile = row_contexts[row_id]["soccer_profile_bundle"]
...
def summarize_bayesian_info(candidate, df, col_profiles, candidate_stats, similarity_info, soccer_consistency_info):
    ...
    rule_signal = min(
        5.0,
        len(candidate.get("violated_rules", [])) + candidate.get("conflict_score", 0.0) / 100.0
    )
    ...
    if semantic_type in {
        "player_given_name", "player_surname", "manager_name",
        "team_entity", "stadium_entity", "city_location", "birthplace_location"
    }:
        rarity *= 0.35
    ...
    neighbor_signal = (
        0.0
        if neighbor_value is None or neighbor_ratio is None
        else (neighbor_ratio if neighbor_value != value else 0.0)
    )

    soccer_signal = soccer_consistency_info.get("soccer_consistency_score", 0.0)
    ...
    posterior = sigmoid(
        ...
        + W_SOCCER_CONSISTENCY * soccer_signal
    )
    ...
    return {
        ...
        "bayes_evidence": {
            ...
            "soccer_consistency_signal": round(float(soccer_signal), 6),
        },
        "note": "posterior is a heuristic Bayesian-style approximation",
    }


def build_soccer_attribution_features(row_ctx: Dict[str, Any], col: str, value: Any, conflict_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    age_conflict = int(
        "birthyear_season_age_consistency" in rule_types
        or (age is not None and (age < PLAYER_MIN_AGE_AT_SEASON or age > PLAYER_MAX_AGE_AT_SEASON))
    )
    team_context_conflict = int("team_context_dominant" in rule_types)
    format_signal = int(len(rule_types & SOCCER_FORMAT_RULE_TYPES) > 0)

    # v2: 这里只把 functional_dependency 计入 fd_like_signal。
    ...
    elif col in PRIMARY_TARGET_COLUMNS:
...
def build_llm_context_text(
    ...
    lines += [
        "",
        "Soccer profile bundle:",
        f"- {row_ctx['soccer_profile_bundle']}",
        "",
        "Column context:",
        ...
        f"- soccer_value_features: {col_ctx['soccer_value_features']}",
    ...
    lines += [
        ...
        f"- evidence_only_fd_count: {conflict_ctx.get('evidence_only_fd_count', 0)}",
        f"- context_rule_count: {conflict_ctx['context_rule_count']}",
        f"- soccer_domain_rule_count: {conflict_ctx['soccer_domain_rule_count']}",
        f"- soccer_format_rule_count: {conflict_ctx['soccer_format_rule_count']}",
        f"- soccer_numeric_rule_count: {conflict_ctx['soccer_numeric_rule_count']}",
        f"- soccer_consistency_rule_count: {conflict_ctx['soccer_consistency_rule_count']}",
        "- violated rules:",
    ]
    ...
    if soccer_features is not None:
        lines += [
            "",
            "Soccer attribution features:",
            f"- target_is_primary_column: {soccer_features.get('target_is_primary_column')}",
            f"- target_is_context_only_column: {soccer_features.get('target_is_context_only_column')}",
            f"- birthyear_value_invalid: {soccer_features.get('birthyear_value_invalid')}",
            f"- season_value_invalid: {soccer_features.get('season_value_invalid')}",
            f"- position_value_invalid: {soccer_features.get('position_value_invalid')}",
            f"- position_needs_canonicalization: {soccer_features.get('position_needs_canonicalization')}",
            f"- birthyear_season_age_conflict: {soccer_features.get('birthyear_season_age_conflict')}",
            f"- team_context_conflict: {soccer_features.get('team_context_conflict')}",
            f"- format_rule_signal: {soccer_features.get('format_rule_signal')}",
            f"- fd_like_signal: {soccer_features.get('fd_like_signal')}",
            f"- row_age_at_season: {soccer_features.get('row_age_at_season')}",
            f"- soccer_attribution_bucket: {soccer_features.get('soccer_attribution_bucket')}",
        ]
    ...
    for sr in sim_ctx["topk_similar_rows"]:
        if "row_values" in sr:
            ...
            lines.append(
                f"  - row_id={sr['row_id']}, similarity={sr['similarity']}, "
                f"target_column_value={sr['target_column_value']}"
            )

    lines += [
        "",
        "Soccer consistency evidence:",
        f"- evidence_flags: {soccer_ctx['evidence_flags']}",
        f"- team_season_distribution: {soccer_ctx['team_season_distribution']}",
        f"- team_distribution: {soccer_ctx['team_distribution']}",
        f"- stadium_city_distribution: {soccer_ctx['stadium_city_distribution']}",
        f"- soccer_consistency_score: {soccer_ctx['soccer_consistency_score']}",
        "",
...
def _build_flat_row(cand, row_ctx, col_ctx, conflict_ctx, sim_ctx, soccer_ctx, bayes_ctx):
    ...
    return {
        ...
        "soccer_domain_rule_count": conflict_ctx["soccer_domain_rule_count"],
        "soccer_format_rule_count": conflict_ctx["soccer_format_rule_count"],
        "soccer_numeric_rule_count": conflict_ctx["soccer_numeric_rule_count"],
        "soccer_consistency_rule_count": conflict_ctx["soccer_consistency_rule_count"],
        ...
        "row_birthyear_int": spb["birthyear_int"],
        "row_season_int": spb["season_int"],
        "row_age_at_season": spb["age_at_season"],
        "row_position_canonical": spb["position_canonical"],
        ...
        "adult_profile_inconsistency_count": len(spb["soccer_profile_inconsistencies"]),
        "soccer_profile_inconsistency_count": len(spb["soccer_profile_inconsistencies"]),

        "soccer_consistency_score": soccer_ctx["soccer_consistency_score"],
        "soccer_evidence_flags": "|".join(soccer_ctx["evidence_flags"]),

        # 兼容旧 adult 字段
        "adult_consistency_score": soccer_ctx["soccer_consistency_score"],
        "adult_evidence_flags": "|".join(soccer_ctx["evidence_flags"]),
...
def _build_one_context(cand: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    soccer_ctx = summarize_soccer_consistency_info_fast(
        G_DF,
        row_id,
        col,
        value,
    ...
    bayes_ctx = summarize_bayesian_info(cand, G_DF, G_COL_PROFILES, G_CANDIDATE_STATS, sim_ctx, soccer_ctx)
    soccer_features = build_soccer_attribution_features(row_ctx, col, value, conflict_ctx)
    ...
    item = {
        ...
        "soccer_consistency_context": soccer_ctx,
        "soccer_attribution_features": soccer_features,

        # 兼容旧 adult 字段
        "adult_consistency_context": soccer_ctx,

        "bayesian_context": bayes_ctx,
        "llm_context_text": llm_context_text,
    }

    flat_row = _build_flat_row(cand, row_ctx, col_ctx, conflict_ctx, sim_ctx, soccer_ctx, bayes_ctx)
    flat_row.update(soccer_features)
...
def _merge_parts(part_infos: List[Dict[str, Any]]):
    ...
    flat_parts = [pd.read_csv(info["csv_part"]) for info in part_infos]
...
def main():
    ...
    global G_TEAM_SEASON_MAP, G_TEAM_MAP, G_STADIUM_CITY_MAP, G_NEIGHBOR_INDICES
    ...
    print("[4/8] Precomputing Soccer group distributions ...")
    team_season_map, team_map, stadium_city_map = precompute_soccer_group_distributions(df)
    print(
        f"  team-season groups = {len(team_season_map)}, "
        f"team groups = {len(team_map)}, "
        f"stadium-city groups = {len(stadium_city_map)}"
    )

    print("[5/8] Precomputing similar rows only for candidate rows ...")
    X = build_similarity_matrix(df, col_profiles)
    candidate_row_ids = sorted(set(int(c["row_id"]) for c in candidates))
    neighbor_indices = precompute_neighbor_indices_for_candidate_rows(
        X,
        candidate_row_ids=candidate_row_ids,
        top_k=TOP_K_SIMILAR_ROWS,
    )
    print(f"  candidate row ids = {len(candidate_row_ids)}")
    ...
    G_TEAM_SEASON_MAP = team_season_map
    G_TEAM_MAP = team_map
    G_STADIUM_CITY_MAP = stadium_city_map
    ...
    if NUM_WORKERS <= 1:
        for i, chunk_args in enumerate(chunks, start=1):
            ...
            print(
                f"  finished chunks {i}/{len(chunks)}, "
                f"contexts={sum(x['n'] for x in part_infos)}, "
