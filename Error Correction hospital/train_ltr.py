...
def parse_args():
    ...
    p.add_argument("--pairwise_responses", default="repair_pairwise_responses.jsonl")
    p.add_argument("--output_dir", default="pairwise_ltr_student_output_iter_f1_plus")
...
COLUMN_WEIGHT_MAP = {
    ...
    "MeasureName": 2.8,
    "Condition": 2.2,
    # 当前 FN 很高但模式稳定的列，适当加权帮助恢复 recall
    "PhoneNumber": 2.1,
    "ZipCode": 2.1,
    "ProviderNumber": 2.1,
    "State": 1.9,
    "EmergencyService": 1.9,
    "HospitalType": 1.9,
    "HospitalOwner": 1.9,
    "CountyName": 2.0,
    "City": 1.8,
...
def build_candidate_index(df: pd.DataFrame, feature_columns: List[str]):
    ...
    for _, row in df.iterrows():
        key = (int(row["row_id"]), str(row["column"]), canonical_empty_text(row["candidate_value"]))
        idx[key] = {
    ...
    for x in rows:
        ...
        try:
            cleaned.append({
                "row_id": int(x["row_id"]),
                "column": str(x["column"]),
                "dirty_value": canonical_empty_text(x.get("dirty_value", "")),
                "candidate_a_value": canonical_empty_text(x.get("candidate_a_value", "")),
                "candidate_b_value": canonical_empty_text(x.get("candidate_b_value", "")),
...
class PairwiseRankingDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        key_a = (int(row["row_id"]), str(row["column"]), canonical_empty_text(row["candidate_a_value"]))
        key_b = (int(row["row_id"]), str(row["column"]), canonical_empty_text(row["candidate_b_value"]))
        sample_weight = max(
