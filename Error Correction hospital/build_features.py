...
# 上一步新版候选展开文件
CANDIDATES_FILE = "repair_candidates_expanded_v2.csv"
...
# 原始脏数据表：用于在没有现成共现字典时，构建经验共现字典
RAW_DATA_FILE = "/mnt/mydata/dq/projects/Splittree/hospital/hospital_dirty_no_address23.csv"
...
def load_or_build_cooccurrence_dict():
    ...
    with open(OUTPUT_BUILT_COOCCUR_JSON, "w", encoding="utf-8-sig") as f:
        json.dump(cooccur, f, ensure_ascii=False)

    print(f"[INFO] 已保存构建出的共现字典: {OUTPUT_BUILT_COOCCUR_JSON}")
    return cooccur
