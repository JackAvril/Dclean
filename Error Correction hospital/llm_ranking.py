...
# 输入1：候选特征文件（新版）
INPUT_CANDIDATE_FEATURES_CSV = "repair_candidate_features_v2.csv"

# 输入2：详细上下文（rich jsonl）
INPUT_DETAIL_JSONL = "/mnt/mydata/dq/projects/Splittree/test/Error Detection new/candidate_llm_contexts_v2.jsonl"
...
HARD_COLUMNS = {"MeasureCode", "Stateavg", "Condition", "MeasureName", "Score", "Sample"}
DROP_UNKNOWN_IF_TOO_MANY = False

# Pairwise 策略
PAIRWISE_STRATEGY = "all_pairs"   # 可选: all_pairs / one_vs_top
...
# 若你只想先导出采样与 pairwise 请求，不实际调用 LLM，可设为 True
...
# ============================================================
# 3. 构建 cell-level 摘要（从 candidate-level 特征聚合）
# ============================================================
    ...
    for (row_id, column), g in grouped:
        ...
        if "final_score_from_candidate_gen" in g.columns:
            if "candidate_rank" in g.columns:
                g = g.sort_values(["final_score_from_candidate_gen", "candidate_rank"], ascending=[False, True])
            else:
                g = g.sort_values(["final_score_from_candidate_gen"], ascending=[False])
        elif "final_score" in g.columns:
            if "candidate_rank" in g.columns:
                g = g.sort_values(["final_score", "candidate_rank"], ascending=[False, True])
            else:
                g = g.sort_values(["final_score"], ascending=[False])
        elif "candidate_rank" in g.columns:
            g = g.sort_values(["candidate_rank"], ascending=[True])

        dirty_value = first_non_null(g["dirty_value"], "")
        ...
        score_col = "final_score_from_candidate_gen" if "final_score_from_candidate_gen" in g.columns else ("final_score" if "final_score" in g.columns else None)
        ...
        avg_rule_support = pd.to_numeric(
            g["candidate_rule_support_ratio"] if "candidate_rule_support_ratio" in g.columns else pd.Series([0] * len(g)),
            errors="coerce"
        ).fillna(0).mean()

        max_rule_support = pd.to_numeric(
            g["candidate_rule_support_ratio"] if "candidate_rule_support_ratio" in g.columns else pd.Series([0] * len(g)),
            errors="coerce"
        ).fillna(0).max()

        avg_bayes = pd.to_numeric(
            g["bayes_mean_cond_prob"] if "bayes_mean_cond_prob" in g.columns else pd.Series([0] * len(g)),
            errors="coerce"
        ).fillna(0).mean()

        max_bayes = pd.to_numeric(
            g["bayes_mean_cond_prob"] if "bayes_mean_cond_prob" in g.columns else pd.Series([0] * len(g)),
            errors="coerce"
        ).fillna(0).max()

        avg_edit_sim = pd.to_numeric(
            g["edit_similarity_to_dirty"] if "edit_similarity_to_dirty" in g.columns else pd.Series([0] * len(g)),
            errors="coerce"
        ).fillna(0).mean()

        max_edit_sim = pd.to_numeric(
            g["edit_similarity_to_dirty"] if "edit_similarity_to_dirty" in g.columns else pd.Series([0] * len(g)),
            errors="coerce"
        ).fillna(0).max()

        source_count_top1 = int(g["source_count"].iloc[0]) if "source_count" in g.columns and len(g) >= 1 and pd.notna(g["source_count"].iloc[0]) else 0
        top1_is_from_rule = int(g["is_from_rule"].iloc[0]) if "is_from_rule" in g.columns and len(g) >= 1 else 0
        top1_is_from_neighbor = int(g["is_from_neighbor"].iloc[0]) if "is_from_neighbor" in g.columns and len(g) >= 1 else 0
        top1_is_from_similarity = int(g["is_from_similarity"].iloc[0]) if "is_from_similarity" in g.columns and len(g) >= 1 else 0
        top1_is_from_hint = int(g["is_from_hint"].iloc[0]) if "is_from_hint" in g.columns and len(g) >= 1 else 0
        top1_is_from_dictionary = int(g["is_from_dictionary"].iloc[0]) if "is_from_dictionary" in g.columns and len(g) >= 1 else 0
        top1_is_from_pattern_restore = int(g["is_from_pattern_restore"].iloc[0]) if "is_from_pattern_restore" in g.columns and len(g) >= 1 else 0
        ...
        rows.append({
            ...
            "avg_rule_support": float(avg_rule_support),
            "max_rule_support": float(max_rule_support),
            "avg_bayes": float(avg_bayes),
            "max_bayes": float(max_bayes),
            "avg_edit_sim": float(avg_edit_sim),
            "max_edit_sim": float(max_edit_sim),
            ...
            "is_hard_column": int(str(column) in HARD_COLUMNS),
        })
...
def build_candidate_list_for_cell(cell_key: Tuple[int, str], cand_df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    if DROP_UNKNOWN_IF_TOO_MANY and len(g) > limit:
        if "candidate_value" in g.columns:
    ...
    for _, r in g.iterrows():
        item = {
            "candidate_value": safe_jsonable(r.get("candidate_value")),
            "candidate_rank": safe_jsonable(r.get("candidate_rank")),
            "candidate_score": safe_jsonable(r.get(score_col)) if score_col else None,
            "candidate_source": safe_jsonable(r.get("candidate_source")),
            "source_count": safe_jsonable(r.get("source_count")),
            "source_score": safe_jsonable(r.get("source_score")),
            "is_from_rule": safe_jsonable(r.get("is_from_rule")),
            "is_from_neighbor": safe_jsonable(r.get("is_from_neighbor")),
            "is_from_similarity": safe_jsonable(r.get("is_from_similarity")),
            "is_from_hint": safe_jsonable(r.get("is_from_hint")),
            "is_from_dictionary": safe_jsonable(r.get("is_from_dictionary")),
            "is_from_pattern_restore": safe_jsonable(r.get("is_from_pattern_restore")),
            "is_rule_and_dictionary_supported": safe_jsonable(r.get("is_rule_and_dictionary_supported")),
            "candidate_rule_support_ratio": safe_jsonable(r.get("candidate_rule_support_ratio")),
            "rule_support_gain": safe_jsonable(r.get("rule_support_gain")),
            "candidate_matches_similar_row_target": safe_jsonable(r.get("candidate_matches_similar_row_target")),
            "candidate_match_ratio_in_similar_rows": safe_jsonable(r.get("candidate_match_ratio_in_similar_rows")),
            "bayes_mean_cond_prob": safe_jsonable(r.get("bayes_mean_cond_prob")),
            "bayes_max_cond_prob": safe_jsonable(r.get("bayes_max_cond_prob")),
            "edit_similarity_to_dirty": safe_jsonable(r.get("edit_similarity_to_dirty")),
            "avg_keyboard_distance": safe_jsonable(r.get("avg_keyboard_distance")),
            "phonetic_similarity_to_dirty": safe_jsonable(r.get("phonetic_similarity_to_dirty")),
            "candidate_in_column_top_values": safe_jsonable(r.get("candidate_in_column_top_values")),
            "candidate_looks_like_measurecode": safe_jsonable(r.get("candidate_looks_like_measurecode")),
            "candidate_looks_like_stateavg": safe_jsonable(r.get("candidate_looks_like_stateavg")),
            "candidate_family_match_dirty": safe_jsonable(r.get("candidate_family_match_dirty")),
            "candidate_suffix_match_dirty": safe_jsonable(r.get("candidate_suffix_match_dirty")),
            "candidate_state_prefix_match_row_state": safe_jsonable(r.get("candidate_state_prefix_match_row_state")),
            "candidate_stateavg_matches_row_measurecode": safe_jsonable(r.get("candidate_stateavg_matches_row_measurecode")),
            "candidate_condition_matches_measurecode_family": safe_jsonable(r.get("candidate_condition_matches_measurecode_family")),
            "candidate_name_anchor_overlap_with_condition": safe_jsonable(r.get("candidate_name_anchor_overlap_with_condition")),
            "candidate_name_anchor_overlap_with_measurecode_family": safe_jsonable(r.get("candidate_name_anchor_overlap_with_measurecode_family")),
            "candidate_is_valid_percent": safe_jsonable(r.get("candidate_is_valid_percent")),
            "dirty_is_valid_percent": safe_jsonable(r.get("dirty_is_valid_percent")),
            "candidate_percent_restoration_gain": safe_jsonable(r.get("candidate_percent_restoration_gain")),
            "candidate_is_valid_sample_pattern": safe_jsonable(r.get("candidate_is_valid_sample_pattern")),
            "dirty_is_valid_sample_pattern": safe_jsonable(r.get("dirty_is_valid_sample_pattern")),
            "candidate_sample_restoration_gain": safe_jsonable(r.get("candidate_sample_restoration_gain")),
        }
...
通用判断原则：
1. 优先考虑规则一致性（FD/依赖/约束是否支持该候选）
2. 再考虑上下文一致性（与同行其他属性是否更匹配）
3. 再考虑统计支持（列内高频值、共现概率、邻居支持、similar row 支持）
4. 再考虑字符串变换合理性（拼写相似、键盘误触、发音相似、pattern 恢复）
    ...
    for _, row in sampled_df.iterrows():
        ...
        for pair_idx, (i, j) in enumerate(pair_ids):
            ...
            req = {
                ...
                "candidate_a_index": i,
                "candidate_b_index": j,
                "candidate_a_value": cand_a["candidate_value"],
                "candidate_b_value": cand_b["candidate_value"],
...
def get_client() -> OpenAI:
    if not API_KEY:
        raise ValueError("未设置 DEEPSEEK_API_KEY / API_KEY，无法调用 LLM")
...
def main():
    ...
    # 3) 生成 pairwise 请求
    requests = build_pairwise_requests(sampled_df, cand_df, detail_map)
