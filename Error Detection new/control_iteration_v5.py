...
TRAIN_SCRIPT_PATH = "train_mlp_v5.py"
INFER_SCRIPT_PATH = "infer_mlp_v5.py"
...
def select_hard_cases_for_llm(records: List[Dict[str, Any]], max_cases: int) -> List[Dict[str, Any]]:
    ...
    def score_fn(r: Dict[str, Any]) -> float:
        ...
        score += 4.5 * int(flags.get("targeted_rescue_hardcase", 0))
        score += 3.5 * int(flags.get("recall_recovery_hardcase", 0))
        score += 1.5 * int(flags.get("is_stage_disagree", 0))
