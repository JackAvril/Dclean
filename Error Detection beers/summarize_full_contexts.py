...
def analyze_columns(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    for col in df.columns:
        ...
        if col in NUMERIC_SEMANTIC_COLUMNS:
            parsed = non_null.map(lambda x: parse_numeric_by_column(col, x))
            nums = parsed.dropna()
            numeric_ratio = parsed.notna().mean() if len(parsed) > 0 else 0.0
            numeric_vals = nums
        else:
        ...
        if detected_type == "numeric":
            nums = pd.Series(numeric_vals).dropna()
            if len(nums) > 0:
                numeric_stats = {
                    "min": float(nums.min()),
                    "max": float(nums.max()),
...
def summarize_column_context(col: str, value: Any, col_profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    numeric_value = parse_numeric_by_column(col, value) if col in NUMERIC_SEMANTIC_COLUMNS else try_parse_one_numeric(value)
...
def summarize_conflict_info(candidate: Dict[str, Any], col_profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    numeric_window_rule_count = rule_type_counter.get("numeric_range_window", 0)
    ...
    return {
        ...
        "numeric_window_rule_count": numeric_window_rule_count,
...
def single_cell_similarity(a, b, semantic_type: str, col: str = None):
    ...
    if semantic_type == "numeric":
        an = parse_numeric_by_column(col, a) if col in NUMERIC_SEMANTIC_COLUMNS else try_parse_one_numeric(a)
        bn = parse_numeric_by_column(col, b) if col in NUMERIC_SEMANTIC_COLUMNS else try_parse_one_numeric(b)
...
def row_similarity(target_row: pd.Series, other_row: pd.Series, col_profiles: Dict[str, Dict[str, Any]],
    ...
    for c in target_row.index:
        if ignore_col is not None and c == ignore_col:
            continue
        sim = single_cell_similarity(target_row[c], other_row[c], col_profiles[c]["semantic_type"], c)
        if sim is None:
            continue
        score += sim
        count += 1
...
def build_llm_context_text(
    ...
    lines.append(f"- numeric_snapshot: {row_ctx['numeric_snapshot']}")
    ...
    lines.append(f"- numeric_stats: {col_ctx['numeric_stats']}")
    ...
    lines.append(f"- numeric_window_rule_count: {conflict_ctx['numeric_window_rule_count']}")
    ...
    with open(OUTPUT_JSONL, "w", encoding="utf-8") as f:
        for cand in candidates:
            ...
            flat_rows.append({
                ...
                "numeric_window_rule_count": conflict_ctx["numeric_window_rule_count"],
            })
