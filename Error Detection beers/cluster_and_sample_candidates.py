...
def build_numeric_window_bucket(detail_obj: Dict[str, Any]) -> str:
    try:
        conflict_ctx = detail_obj.get("conflict_context", {})
        cnt = int(conflict_ctx.get("numeric_window_rule_count", 0))
        if cnt <= 0:
...
def load_summary_csv(summary_csv: str, detail_map: Dict[Tuple[int, str], Dict[str, Any]]) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        numeric_window_buckets.append(build_numeric_window_bucket(detail_obj))

    df["main_rule_type"] = main_rule_types
    df["main_usage_role"] = main_usage_roles
    df["pattern_bucket"] = pattern_buckets
    df["numeric_window_bucket"] = numeric_window_buckets
    ...
    df["numeric_value_bucket"] = df.apply(
        lambda r: build_numeric_value_bucket(r.get("column"), r.get("numeric_value")),
        axis=1
    )
    ...
    for bucket_id, bucket_df in summary_df.groupby("bucket_id", sort=False):
        ...
        for cluster_id, cluster_df in bucket_clustered.groupby("cluster_id", sort=False):
            ...
            bucket_summary_rows.append({
                ...
                "numeric_window_bucket": str(cluster_df["numeric_window_bucket"].iloc[0]) if "numeric_window_bucket" in cluster_df.columns else "unknown",
                "numeric_value_bucket": str(cluster_df["numeric_value_bucket"].iloc[0]) if "numeric_value_bucket" in cluster_df.columns else "unknown",
            })
        ...
        for _, row in sampled_df.iterrows():
            ...
            out = {
                ...
                "summary_context": {
                    ...
                    "numeric_window_rule_count": row.get("numeric_window_rule_count"),
                    "pattern_bucket": row.get("pattern_bucket"),
                    "rarity_bucket": row.get("rarity_bucket"),
                    "neighbor_bucket": row.get("neighbor_bucket"),
                    "numeric_window_bucket": row.get("numeric_window_bucket"),
                    "numeric_value_bucket": row.get("numeric_value_bucket"),
                },
