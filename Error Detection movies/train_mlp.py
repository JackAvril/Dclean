...
INPUT_CSV = "final_train_dataset_movies.csv"
OUTPUT_DIR = "two_stage_model_output_movies_v4_with_verifier"

DETECTOR_TRAIN_MODE = "full"
DETECTOR_SOURCE_DIR = "two_stage_model_output_movies_v4_with_verifier"
...
STAGE2_FALSE_POS_RISK_COLS = {"Release Date", "Year", "Duration", "RatingValue", "RatingCount", "ReviewCount", "Actors", "Cast"}
...
VERIFIER_MEASURE_COL_FP_BOOST = 1.8
...
STAGE1_NUMERIC_FEATURES = [
    ...
    'neighbor_majority_ratio', 'prior_error_probability', 'posterior_error_probability', 'is_equal_to_neighbor_majority',
    'neighbor_agreement_score', 'strong_rule_count', 'candidate_generation_rule_count', 'fd_like_count',
    'context_rule_count', 'global_rule_count', 'rare_value_count', 'typo_rule_count',
    'pattern_rule_count', 'schema_rule_count', 'movie_typed_rule_count', 'movie_consistency_rule_count',
    'candidate_rule_ratio', 'rule_total_count', 'strong_rule_ratio', 'context_rule_ratio',
    'fd_like_ratio', 'global_rule_ratio', 'movie_typed_rule_ratio', 'movie_consistency_rule_ratio',
    'movie_rule_total_count', 'has_movie_rule_signal', 'rule_strength_sum', 'has_neighbor_signal',
    'high_neighbor_consistency', 'is_rare_value', 'neighbor_disagree', 'parsed_year',
    'parsed_release_year', 'duration_minutes', 'rating_value_float', 'count_value_int',
    'review_total', 'list_token_count', 'has_mojibake_or_control_chars', 'movie_row_year',
    'movie_row_release_year', 'movie_row_release_year_gap', 'movie_row_duration_minutes', 'movie_row_rating_value_float',
    'movie_row_rating_count_int', 'movie_row_review_total', 'movie_row_review_to_rating_ratio', 'movie_row_actors_cast_overlap',
    'is_movie_time_col', 'is_movie_rating_col', 'is_movie_people_col', 'is_movie_list_col',
    'is_movie_text_col',
]

STAGE1_CATEGORICAL_FEATURES = [
    'column', 'semantic_type', 'detected_type', 'main_rule_type',
    'main_usage_role', 'pattern_bucket', 'rarity_bucket', 'neighbor_bucket',
    'candidate_rule_dominant', 'movie_rule_bucket', 'movie_field_bucket', 'year_bucket',
    ...
    'parsed_release_country', 'movie_row_release_country', 'list_token_count_bucket', 'text_length_bucket',
]

STAGE2_NUMERIC_FEATURES = [
    'raw_pred_error_prob', 'raw_pred_label', 'raw_margin_to_threshold', 'stage1_high_conf_error',
    'stage1_mid_zone', 'violation_count', 'conflict_score', 'value_frequency',
    'value_frequency_rank', 'neighbor_majority_ratio', 'prior_error_probability', 'posterior_error_probability',
    'is_equal_to_neighbor_majority', 'neighbor_agreement_score', 'strong_rule_count', 'candidate_generation_rule_count',
    'fd_like_count', 'context_rule_count', 'global_rule_count', 'rare_value_count',
    'typo_rule_count', 'pattern_rule_count', 'schema_rule_count', 'movie_typed_rule_count',
    'movie_consistency_rule_count', 'candidate_rule_ratio', 'rule_total_count', 'strong_rule_ratio',
    'context_rule_ratio', 'fd_like_ratio', 'global_rule_ratio', 'movie_typed_rule_ratio',
    'movie_consistency_rule_ratio', 'movie_rule_total_count', 'has_movie_rule_signal', 'rule_strength_sum',
    'has_neighbor_signal', 'high_neighbor_consistency', 'is_rare_value', 'neighbor_disagree',
    'parsed_year', 'parsed_release_year', 'duration_minutes', 'rating_value_float',
    'count_value_int', 'review_total', 'list_token_count', 'has_mojibake_or_control_chars',
    'movie_row_year', 'movie_row_release_year', 'movie_row_release_year_gap', 'movie_row_duration_minutes',
    'movie_row_rating_value_float', 'movie_row_rating_count_int', 'movie_row_review_total', 'movie_row_review_to_rating_ratio',
    'movie_row_actors_cast_overlap', 'is_movie_time_col', 'is_movie_rating_col', 'is_movie_people_col',
    'is_movie_list_col', 'is_movie_text_col', 'rare_only_signal', 'is_weak_only',
]
...
VERIFIER_NUMERIC_FEATURES = [
    'raw_pred_error_prob', 'raw_pred_label', 'raw_margin_to_threshold', 'stage1_high_conf_error',
    'stage1_mid_zone', 'detector_pred_error_prob', 'detector_pred_label', 'detector_margin_to_threshold',
    'is_stage_disagree', 'violation_count', 'conflict_score', 'value_frequency',
    'value_frequency_rank', 'neighbor_majority_ratio', 'prior_error_probability', 'posterior_error_probability',
    'is_equal_to_neighbor_majority', 'neighbor_agreement_score', 'strong_rule_count', 'candidate_generation_rule_count',
    'fd_like_count', 'context_rule_count', 'global_rule_count', 'rare_value_count',
    'typo_rule_count', 'pattern_rule_count', 'schema_rule_count', 'movie_typed_rule_count',
...
def get_verifier_recall_rescue_mask(df: pd.DataFrame) -> pd.Series:
    ...
    movie_rule_cnt = pd.to_numeric(out.get("movie_rule_total_count", 0), errors="coerce").fillna(0.0)

    common_gate = raw_label.eq(1) & detector_label.eq(0) & (raw_prob >= 0.85) & (posterior >= 0.75)
    structural_mask = (
        col.isin(["Year", "Release Date", "Duration", "RatingValue", "RatingCount", "ReviewCount", "Actors", "Cast"]) &
        (movie_rule_cnt > 0) &
        (conflict >= 2.0)
    )
    fd_neighbor_mask = rule.isin(["functional_dependency", "soft_functional_dependency", "dominant_value_by_context", "dominant_value_by_context_pair"]) & nb.eq("strong_support_other")
    near_miss_mask = detector_prob >= 0.45
    return (common_gate & (structural_mask | fd_neighbor_mask) & near_miss_mask).astype(bool)
...
def build_verifier_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out.loc[fp_mask & rule_series.isin(["functional_dependency", "soft_functional_dependency"]) & nb_series.isin(["strong_support_current", "weak_support_current"]), "verifier_sample_weight"] *= VERIFIER_FD_CONFLICT_FP_BOOST
    out.loc[fp_mask & (rare_only_signal == 1), "verifier_sample_weight"] *= VERIFIER_RARE_ONLY_FP_BOOST
    out.loc[fp_mask & detector_prob.between(0.45, 0.85), "verifier_sample_weight"] *= VERIFIER_BORDERLINE_FP_BOOST
    out.loc[fp_mask & col_series.isin({"Release Date", "Year", "Duration", "RatingValue", "RatingCount", "ReviewCount", "Actors", "Cast", "Language", "Country", "Genre"}), "verifier_sample_weight"] *= VERIFIER_RISK_COLUMN_FP_BOOST
    out.loc[fp_mask & nb_series.eq("weak_support_other"), "verifier_sample_weight"] *= VERIFIER_WEAK_OTHER_FP_BOOST
    out.loc[fp_mask & nb_series.eq("weak_support_other") & rule_series.isin(["rare_value", "rare_pattern", "functional_dependency", "soft_functional_dependency"]), "verifier_sample_weight"] *= VERIFIER_WEAK_OTHER_RARE_FD_FP_BOOST
    out.loc[fp_mask & (movie_rule_cnt > 0), "verifier_sample_weight"] *= 1.35

    true_error_mask = (out["verifier_label"] == 1) & col_series.isin({"Year", "Release Date", "Duration", "RatingValue", "RatingCount", "ReviewCount", "Actors", "Cast"})
    out.loc[true_error_mask, "verifier_sample_weight"] *= VERIFIER_SAMPLE_TRUE_ERROR_BOOST

    rescue_candidate_mask = pd.to_numeric(out.get("is_recall_rescue_candidate", 0), errors="coerce").fillna(0).astype(int).eq(1)
    out.loc[rescue_candidate_mask & (out["verifier_label"] == 1), "verifier_sample_weight"] *= VERIFIER_RESCUE_CANDIDATE_TRUE_ERROR_BOOST
    out.loc[rescue_candidate_mask & (out["verifier_label"] == 0), "verifier_sample_weight"] *= VERIFIER_RESCUE_CANDIDATE_FP_DAMP
