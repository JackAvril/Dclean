...
def parse_time_like(value) -> Optional[int]:
    ...
    for junk in [
        "(estimated runway)", "(estimated)", "(actual)", "(gate)", "(runway)",
        "(+8:00)", "(-00:00)"
    ]:
...
def analyze_columns(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    for col in df.columns:
        ...
        if col in TIME_COLUMNS:
            detected_type = "time_like"
            semantic_type = "time_like"
        else:
        ...
        time_stats = None
        if col in TIME_COLUMNS:
            time_values = non_null.map(parse_time_like).dropna()
            if len(time_values) > 0:
                tv = time_values.astype(float)
                time_stats = {
                    "min_minutes": float(tv.min()),
                    "max_minutes": float(tv.max()),
                    "mean_minutes": float(tv.mean()),
                    "median_minutes": float(tv.quantile(0.5)),
                    "q1_minutes": float(tv.quantile(0.25)),
                    "q3_minutes": float(tv.quantile(0.75)),
                }
...
def summarize_row_context(df: pd.DataFrame, row_id: int, col_profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if all(c in row for c in ["sched_dep_time", "sched_arr_time"]):
        sd = parse_time_like(row.get("sched_dep_time"))
        sa = parse_time_like(row.get("sched_arr_time"))
        if sd is not None and sa is not None:
            d = sa - sd
            if d < 0:
                d += 1440
            time_bundle["sched_duration_minutes"] = d
        else:
            time_bundle["sched_duration_minutes"] = None

    if all(c in row for c in ["act_dep_time", "act_arr_time"]):
        ad = parse_time_like(row.get("act_dep_time"))
        aa = parse_time_like(row.get("act_arr_time"))
        if ad is not None and aa is not None:
            d = aa - ad
            if d < 0:
                d += 1440
            time_bundle["act_duration_minutes"] = d
        else:
            time_bundle["act_duration_minutes"] = None
...
def summarize_column_context(col: str, value: Any, col_profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    time_value_minutes = parse_time_like(value) if col in TIME_COLUMNS else None
    ...
    return {
        ...
        "time_stats": prof["time_stats"],
        "time_value_minutes": time_value_minutes,
...
def summarize_conflict_info(candidate: Dict[str, Any], col_profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "time_window_rule_count": time_window_rule_count,
...
def single_cell_similarity(a, b, semantic_type: str):
    ...
    if semantic_type == "time_like":
        at = parse_time_like(a)
        bt = parse_time_like(b)
...
def build_llm_context_text(
    ...
    lines.append(f"- time_bundle: {row_ctx['time_bundle']}")
    ...
    lines.append(f"- time_value_minutes: {col_ctx['time_value_minutes']}")
    lines.append(f"- time_stats: {col_ctx['time_stats']}")
    ...
    lines.append(f"- time_window_rule_count: {conflict_ctx['time_window_rule_count']}")
    ...
    with open(OUTPUT_JSONL, "w", encoding="utf-8") as f:
        for cand in candidates:
            ...
            flat_rows.append({
                ...
                "time_value_minutes": col_ctx["time_value_minutes"],
                ...
                "time_window_rule_count": conflict_ctx["time_window_rule_count"],
            })
