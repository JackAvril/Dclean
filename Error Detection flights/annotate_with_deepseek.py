...
def build_summary_prefix(sample_obj: Dict[str, Any]) -> str:
    ...
    lines.append(f"- time_value_minutes: {summary_ctx.get('time_value_minutes')}")
    ...
    lines.append(f"- time_window_rule_count: {summary_ctx.get('time_window_rule_count')}")
    ...
    lines.append(f"- time_window_bucket: {summary_ctx.get('time_window_bucket')}")
    lines.append(f"- time_value_bucket: {summary_ctx.get('time_value_bucket')}")
        ...
        with open(OUTPUT_JSONL, "r", encoding="utf-8") as f:
            for line in f:
                ...
                final_rows.append({
                    ...
                    "time_window_rule_count": summary_ctx.get("time_window_rule_count"),
                    ...
                    "time_window_bucket": summary_ctx.get("time_window_bucket"),
                    "time_value_bucket": summary_ctx.get("time_value_bucket"),
                })
