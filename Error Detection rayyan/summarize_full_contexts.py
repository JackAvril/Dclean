...
def analyze_columns(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    for col in df.columns:
        ...
        elif col in NUMERIC_METADATA_COLUMNS:
        ...
        date_stats = None
        if col in DATE_COLUMNS:
            date_values = non_null.map(normalize_date_like).dropna()
            if len(date_values) > 0:
                year_counter = Counter([x[:4] for x in date_values if len(x) >= 4])
                date_stats = {
                    "top_years": [{"year": k, "count": v} for k, v in year_counter.most_common(10)],
                    "canonical_example_count": int(len(date_values)),
                }
...
def summarize_column_context(col: str, value: Any, col_profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    date_value = normalize_date_like(value) if col in DATE_COLUMNS else None
    language_canonical = canonicalize_language(value) if col == "article_language" else None
    issn_canonical = extract_issn_digits(value) if col == "journal_issn" else None
    numeric_value = canonicalize_volume_issue(value) if col in NUMERIC_METADATA_COLUMNS else None
    return {
        "column": col,
        ...
        "language_canonical": language_canonical,
        "issn_canonical": issn_canonical,
...
def summarize_conflict_info(candidate: Dict[str, Any], col_profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for item in col_profiles[col]["top_values"]:
        ...
        if v != current_value and v not in seen:
            column_alternative_values.append({"source": "column_top_value", "value": v, "count": item["count"]})
            seen.add(v)
    ...
    context_rule_count = rule_type_counter.get("dominant_value_by_context", 0) + rule_type_counter.get("dominant_value_by_context_pair", 0)
    ...
    pattern_rule_count = (
        ...
        + rule_type_counter.get("issn_format", 0)
        + rule_type_counter.get("issn_loose_format", 0)
        + rule_type_counter.get("pagination_format", 0)
        + rule_type_counter.get("author_list_braced_format", 0)
    )
    schema_rule_count = (
    ...
    canonical_rule_count = (
        ...
        + rule_type_counter.get("date_like_format", 0)
        + rule_type_counter.get("volume_issue_numeric_format", 0)
    )
...
def single_cell_similarity(a, b, semantic_type: str):
    ...
    if semantic_type == "date_like":
        ad = normalize_date_like(a)
        bd = normalize_date_like(b)
        if ad is None or bd is None:
            if detect_basic_pattern(a) == detect_basic_pattern(b):
                return 0.2
            return 0.0
        return 1.0 if ad == bd else 0.0
...
def summarize_bayesian_info(candidate: Dict[str, Any], df: pd.DataFrame, col_profiles: Dict[str, Dict[str, Any]], candidate_stats: Dict[str, Any], similarity_info: Dict[str, Any]) -> Dict[str, Any]:
    col = candidate["column"]
    ...
    logit_post = safe_logit(prior) + W_RULE * rule_signal + W_RARITY * rarity + W_PATTERN * pattern_signal + W_NEIGHBOR * neighbor_signal
...
def build_llm_context_text(row_id: int, col: str, value: Any, row_ctx: Dict[str, Any], col_ctx: Dict[str, Any], conflict_ctx: Dict[str, Any], sim_ctx: Dict[str, Any], bayes_ctx: Dict[str, Any]) -> str:
    lines = []
    ...
    lines.append(f"- metadata_bundle: {row_ctx['metadata_bundle']}")
    ...
    lines.append(f"- numeric_value: {col_ctx['numeric_value']}")
    lines.append(f"- date_value: {col_ctx['date_value']}")
    lines.append(f"- language_canonical: {col_ctx['language_canonical']}")
    lines.append(f"- issn_canonical: {col_ctx['issn_canonical']}")
    lines.append(f"- date_stats: {col_ctx['date_stats']}")
    ...
    lines.append(f"- canonical_rule_count: {conflict_ctx['canonical_rule_count']}")
...
def main():
    ...
    with open(OUTPUT_JSONL, "w", encoding="utf-8") as f:
        for cand in candidates:
            ...
            llm_context_text = build_llm_context_text(row_id, col, value, row_ctx, col_ctx, conflict_ctx, sim_ctx, bayes_ctx)
            ...
            flat_rows.append({
                ...
                "numeric_value": col_ctx["numeric_value"],
                "date_value": col_ctx["date_value"],
                "language_canonical": col_ctx["language_canonical"],
                "issn_canonical": col_ctx["issn_canonical"],
                ...
                "canonical_rule_count": conflict_ctx["canonical_rule_count"],
            })
