...
def build_canonical_bucket(detail_obj: Dict[str, Any]) -> str:
    try:
        conflict_ctx = detail_obj.get("conflict_context", {})
        cnt = int(conflict_ctx.get("canonical_rule_count", 0))
        if cnt <= 0:
...
def load_summary_csv(summary_csv: str, detail_map: Dict[Tuple[int, str], Dict[str, Any]]) -> pd.DataFrame:
    ...
    df["date_value_bucket"] = df.apply(
        lambda r: build_date_value_bucket(r.get("date_value")),
        axis=1
    )

    df["language_bucket"] = df.apply(
        lambda r: build_language_bucket(r.get("language_canonical")),
        axis=1
    )
    ...
    for bucket_id, bucket_df in summary_df.groupby("bucket_id", sort=False):
        ...
        for cluster_id, cluster_df in bucket_clustered.groupby("cluster_id", sort=False):
            ...
            bucket_summary_rows.append({
                ...
                "canonical_bucket": str(cluster_df["canonical_bucket"].iloc[0]) if "canonical_bucket" in cluster_df.columns else "unknown",
                "date_value_bucket": str(cluster_df["date_value_bucket"].iloc[0]) if "date_value_bucket" in cluster_df.columns else "unknown",
                "language_bucket": str(cluster_df["language_bucket"].iloc[0]) if "language_bucket" in cluster_df.columns else "unknown",
            })
        ...
        for _, row in sampled_df.iterrows():
            ...
            out = {
                ...
                "summary_context": {
                    ...
                    "language_canonical": row.get("language_canonical"),
                    "issn_canonical": row.get("issn_canonical"),
                    ...
                    "canonical_rule_count": row.get("canonical_rule_count"),
                    "pattern_bucket": row.get("pattern_bucket"),
                    "rarity_bucket": row.get("rarity_bucket"),
                    "neighbor_bucket": row.get("neighbor_bucket"),
                    "canonical_bucket": row.get("canonical_bucket"),
                    "date_value_bucket": row.get("date_value_bucket"),
