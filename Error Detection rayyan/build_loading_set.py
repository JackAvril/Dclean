...
DEFAULT_CANONICAL_BUCKET = "unknown"
DEFAULT_DATE_VALUE_BUCKET = "unknown"
DEFAULT_LANGUAGE_BUCKET = "unknown"
...
def load_full_candidate_csv(path: str) -> pd.DataFrame:
    ...
    required_cols = [
        ...
        "canonical_rule_count", "numeric_value",
    ]
...
def fill_bucket_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "canonical_bucket" not in df.columns:
        df["canonical_bucket"] = np.nan
    df["canonical_bucket"] = df["canonical_bucket"].fillna("")
    need_fill_canonical = df["canonical_bucket"].eq("")
    df.loc[need_fill_canonical, "canonical_bucket"] = df[need_fill_canonical].apply(build_canonical_bucket, axis=1)

    if "date_value_bucket" not in df.columns:
        df["date_value_bucket"] = np.nan
    df["date_value_bucket"] = df["date_value_bucket"].fillna("")
    need_fill_date_value = df["date_value_bucket"].eq("")
    df.loc[need_fill_date_value, "date_value_bucket"] = df[need_fill_date_value].apply(build_date_value_bucket, axis=1)

    if "language_bucket" not in df.columns:
        df["language_bucket"] = np.nan
    df["language_bucket"] = df["language_bucket"].fillna("")
    need_fill_language = df["language_bucket"].eq("")
    df.loc[need_fill_language, "language_bucket"] = df[need_fill_language].apply(build_language_bucket, axis=1)
...
def add_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["rule_strength_sum"] = (
        ...
        + df["canonical_rule_count"].astype(float)
    )
