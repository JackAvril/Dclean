...
# 新增：对这些列/规则组合做 verifier bypass 或恢复
BYPASS_VERIFIER_DATE_RULES = {
...
def is_normal_journal_issn_variant(s: str) -> bool:
    txt = str(s).strip().lower()
    if txt == "" or txt == "missing":
        return False
    if bool(__import__("re").fullmatch(r'\d{4}-\d{3}[\dx]', txt)):
        return True
    if bool(__import__("re").fullmatch(r'\d{4}-\d{3}[\dx]\s*\(print\)', txt)):
        return True
    if bool(__import__("re").fullmatch(r'\d{4}-\d{3}[\dx]\s*\(print\)\s*\d{4}-\d{3}[\dx]', txt)):
        return True
    return False


def is_normal_pagination_value(s: str) -> bool:
    txt = str(s).strip().lower()
    if txt == "" or txt == "missing":
        return False
    if bool(__import__("re").fullmatch(r'\d+(-\d+)?', txt)):
        return True
    return False
...
def apply_conditional_detector_thresholds(df: pd.DataFrame, base_thresholds: pd.Series) -> pd.Series:
    ...
    lang_mask = (
        col.eq("article_language") &
        rule.isin(["language_canonical_format", "not_null", "functional_dependency", "soft_functional_dependency"]) &
        nb.eq("strong_support_current")
    )
    thresholds.loc[lang_mask] = COND_LOWER_THRESHOLD.get("article_language", base_thresholds.loc[lang_mask])

    issn_mask = (
        col.eq("journal_issn") &
        rule.isin(["issn_canonical_format", "issn_format", "issn_loose_format", "soft_functional_dependency"]) &
        nb.isin(["strong_support_other", "strong_support_current"])
    )
    thresholds.loc[issn_mask] = COND_LOWER_THRESHOLD.get("journal_issn", base_thresholds.loc[issn_mask])

    date_mask = (
        col.eq("article_jcreated_at") &
        rule.isin([
            "date_like_format",
            "date_component_permutation_suspicious",
            "date_ymd_swap_suspicious",
        ...
        nb.isin(["strong_support_other", "strong_support_current", "weak_support_other"])
    )
    thresholds.loc[date_mask] = COND_LOWER_THRESHOLD.get("article_jcreated_at", base_thresholds.loc[date_mask])

    author_mask = (
        col.eq("author_list") &
        rule.isin(["author_list_braced_format", "author_list_encoding_corruption", "soft_functional_dependency"]) &
        nb.eq("strong_support_other")
    )
    thresholds.loc[author_mask] = COND_LOWER_THRESHOLD.get("author_list", base_thresholds.loc[author_mask])
...
def get_group_verifier_threshold(row: pd.Series, default_threshold: float) -> float:
    ...
    if nb == "weak_support_other" and col in {"journal_title", "jounral_abbreviation", "article_pagination"}:
        th = max(th, 0.64)
...
def apply_fp_verifier(
    ...
    risk_col_mask = col_series.isin({"journal_title", "jounral_abbreviation", "journal_issn", "article_language", "article_jcreated_at", "article_pagination", "author_list"})
    ...
    # 关键修改：对 article_jcreated_at + 强日期错位规则，直接 bypass verifier
    bypass_mask = (
        detector_positive &
        col_series.eq("article_jcreated_at") &
        rule_series.isin(BYPASS_VERIFIER_DATE_RULES)
    )

    if bypass_mask.sum() > 0:
        out.loc[bypass_mask, "verifier_decision"] = "bypass_keep_error"
        out.loc[bypass_mask, "verifier_bypass"] = 1
...
def apply_column_post_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value_series = safe_str_series(out, "value", "missing")
    ...
    mask_structural_fp = (
        final_label.eq(1) &
        col.isin(["journal_title", "jounral_abbreviation", "article_pagination"]) &
        nb.eq("weak_support_other") &
        rule.isin(["rare_value", "rare_pattern", "functional_dependency", "soft_functional_dependency"]) &
        (verifier_prob < 0.68)
    )
    out.loc[mask_structural_fp, "final_pred_label"] = 0

    # 2) language rare_value 压误报
    mask_language_rare = (
        final_label.eq(1) &
        col.eq("article_language") &
        rule.eq("rare_value") &
    ...
    out.loc[mask_language_rare, "final_pred_label"] = 0

    # 3) language 轻度恢复 recall
    mask_language_recover = (
        detector_label.eq(1) &
        col.eq("article_language") &
        (verifier_prob >= 0.45)
    )
    out.loc[mask_language_recover, "final_pred_label"] = 1

    # 4) 关键：article_jcreated_at 的强日期规则恢复，不让 verifier 再误杀
    mask_date_recover = (
        detector_label.eq(1) &
        col.eq("article_jcreated_at") &
        rule.isin(RECOVER_DATE_RULES) &
        (
            (safe_num_series(out, "verifier_bypass", 0).astype(int) == 1) |
            (verifier_prob >= 0.40) |
            detector_prob.ge(0.62)
        )
    )
    out.loc[mask_date_recover, "final_pred_label"] = 1

    # 5) journal_issn 正常常见变体直接放行为 correct，压 persistent FP
    mask_issn_normal = (
        final_label.eq(1) &
        col.eq("journal_issn") &
        value_series.map(is_normal_journal_issn_variant)
    )
    out.loc[mask_issn_normal, "final_pred_label"] = 0

    # 6) article_pagination 正常页码形式直接放行为 correct
    mask_pagination_normal = (
        final_label.eq(1) &
        col.eq("article_pagination") &
        value_series.map(is_normal_pagination_value) &
        rule.isin(["pagination_format", "rare_value", "rare_pattern"])
    )
    out.loc[mask_pagination_normal, "final_pred_label"] = 0
...
def mark_hard_cases(df: pd.DataFrame, detector_thresholds: pd.Series) -> pd.DataFrame:
    ...
    out["fp_conflict_fd_hardcase"] = (
        final_label.eq(1) &
        col.isin(["journal_title", "jounral_abbreviation", "journal_issn"]) &
        rule.isin(["functional_dependency", "soft_functional_dependency"]) &
        nb.isin(["strong_support_current", "weak_support_current"])
    ).astype(int)
    ...
    out["recall_recovery_hardcase"] = (
        final_label.eq(0) &
        col.isin(["article_language", "journal_issn", "article_jcreated_at", "author_list"]) &
        detector_prob.between(detector_thresholds - 0.12, detector_thresholds + 1e-9) &
        rule.isin([
            "functional_dependency", "soft_functional_dependency", "not_null",
    ...
    out["persistent_fp_high_risk"] = (
        final_label.eq(1) &
        col.isin(["journal_title", "jounral_abbreviation", "article_pagination", "author_list", "journal_issn"]) &
        nb.isin(["weak_support_other", "strong_support_current"]) &
        rule.isin(["rare_value", "rare_pattern", "functional_dependency", "soft_functional_dependency", "pagination_format"]) &
        pd.notna(verifier_prob) &
        (verifier_prob >= 0.60)
    ).astype(int)
...
def main():
    ...
    if USE_FP_VERIFIER:
        ...
        verifier_bypass = int(safe_num_series(out_df, "verifier_bypass", 0).sum())
