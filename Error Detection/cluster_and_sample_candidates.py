...
# 输入1：错误的大致上下文（用于聚类）
INPUT_SUMMARY_CSV = "candidate_llm_contexts_flat_new.csv"

# 输入2：错误的详细上下文（用于补全给 LLM）
INPUT_DETAIL_JSONL = "candidate_llm_contexts_new.jsonl"

# 输出
OUTPUT_CLUSTERED_CSV = "candidate_clustered_new.csv"
OUTPUT_SAMPLED_CSV = "candidate_sampled_new.csv"
OUTPUT_SAMPLED_JSONL = "candidate_sampled_with_context_new.jsonl"
OUTPUT_BUCKET_SUMMARY_CSV = "bucket_cluster_summary_new.csv"
...
def extract_main_rule_type(detail_obj: Dict[str, Any]) -> str:
    try:
...
def load_summary_csv(summary_csv: str, detail_map: Dict[Tuple[int, str], Dict[str, Any]]) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        main_rule_types.append(extract_main_rule_type(detail_obj))
    ...
    for bucket_id, bucket_df in summary_df.groupby("bucket_id", sort=False):
        ...
        for cluster_id, cluster_df in bucket_clustered.groupby("cluster_id", sort=False):
            ...
            bucket_summary_rows.append({
                ...
                "semantic_type": cluster_df["semantic_type"].iloc[0],
                "main_rule_type": cluster_df["main_rule_type"].iloc[0],
                "pattern_bucket": cluster_df["pattern_bucket"].iloc[0],
                "rarity_bucket": cluster_df["rarity_bucket"].iloc[0],
                "neighbor_bucket": cluster_df["neighbor_bucket"].iloc[0],
            })
