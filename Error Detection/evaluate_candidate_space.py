...
# 这里放 candidate 空间的预测结果
PREDICTION_CSV = "candidate_scored_two_stage_interval_neighbor.csv"

OUTPUT_DIR = "evaluation_candidate_space2"
MERGED_DETAIL_CSV = os.path.join(OUTPUT_DIR, "candidate_eval_detail.csv")
ERROR_CASES_CSV = os.path.join(OUTPUT_DIR, "candidate_error_cases.csv")
COLUMN_METRICS_CSV = os.path.join(OUTPUT_DIR, "candidate_column_metrics.csv")
SUMMARY_JSON = os.path.join(OUTPUT_DIR, "candidate_summary_metrics.json")
...
# 可选：若 prediction 文件里出现重复 key，是否报错
...
def evaluate_candidate_space():
    ...
    # 只评估 prediction 里实际出现过的 candidate keys
    candidate_keys = pred_df[["row_id", "column"]].drop_duplicates().copy()

    gt_candidate_df = gt_df.merge(
        candidate_keys,
        on=["row_id", "column"],
        how="inner",
        validate="one_to_one"
    ...
    if missing_pred > 0:
        print(f"[WARN] candidate 子集中仍有 {missing_pred} 行没有匹配到预测结果，默认补为 0。")
    ...
    summary = {
        ...
        "evaluation_scope": "candidate_space_only",
        "n_rows_table": int(len(dirty_df)),
        "n_columns_table": int(len(dirty_df.columns)),
        "n_total_cells_in_full_table": int(len(gt_df)),
        "n_candidate_cells_evaluated": int(len(merged)),
        "candidate_coverage_over_full_table": float(len(merged) / len(gt_df)) if len(gt_df) > 0 else None,
        "n_true_error_cells_in_candidate": int(merged["true_label"].sum()),
        "n_pred_error_cells_in_candidate": int(merged["pred_label"].sum()),
    ...
    # ---------- 打印 ----------
    print("===== Candidate 空间评估结果 =====")
    print(f"整表单元格总数: {len(gt_df)}")
    print(f"candidate 单元格数: {len(merged)}")
    print(f"candidate 覆盖率: {len(merged) / len(gt_df):.6f}")
    print(f"candidate 中真实错误单元格数: {merged['true_label'].sum()}")
    print(f"candidate 中预测错误单元格数: {merged['pred_label'].sum()}")
    print()
    ...
    print()
    print("===== 文件已保存 =====")
    print(f"candidate 逐单元格评估: {MERGED_DETAIL_CSV}")
    print(f"candidate 错判样本    : {ERROR_CASES_CSV}")
    print(f"candidate 按列指标    : {COLUMN_METRICS_CSV}")
    print(f"candidate 总体 JSON   : {SUMMARY_JSON}")


if __name__ == "__main__":
    evaluate_candidate_space()