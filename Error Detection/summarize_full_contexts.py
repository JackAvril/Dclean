...
INPUT_SHRUNK_CSV = "candidate_cells_new.csv"

OUTPUT_JSONL = "candidate_llm_contexts_new.jsonl"
OUTPUT_CSV = "candidate_llm_contexts_flat_new.csv"
...
def extract_expected_value_from_reason(reason: str):
    ...
    marker = "suggests rhs="
    obs_marker = ", observed="
    if marker in reason and obs_marker in reason:
        try:
            a = reason.index(marker) + len(marker)
            b = reason.index(obs_marker)
            return reason[a:b].strip()
        except Exception:
            return None
    return None
...
def build_llm_context_text(
    ...
    for r in conflict_ctx["violated_rules_detailed"][:20]:
        lines.append(
            ...
            f"priority={r.get('priority')}, expected_value={r.get('expected_value')}, "
