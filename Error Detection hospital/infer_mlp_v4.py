...
def apply_fp_verifier(
    ...
    risk_col_mask = col_series.isin({"ProviderNumber", "PhoneNumber", "Score", "Stateavg", "Address1", "Sample"})
    neighbor_mask = neighbor_series.isin({"strong_support_current", "weak_support_current"})
    ...
    if PRINT_VERIFIER_DEBUG:
        print(f"[DEBUG] verifier samples = {len(keep_prob)}")
        print_verifier_debug_stats(keep_prob, verifier_threshold)

    out.loc[verifier_mask, "verifier_keep_error_prob"] = keep_prob

    group_thresholds = verifier_df.apply(lambda r: get_group_verifier_threshold(r, verifier_threshold), axis=1).values
    out.loc[verifier_mask, "group_verifier_threshold"] = group_thresholds

    decisions = np.where(keep_prob >= group_thresholds, "keep_error", "reject_error")
    out.loc[verifier_mask, "verifier_decision"] = decisions

    final_pred = safe_num_series(out, "detector_pred_label", 0).astype(int).copy()
    reject_mask = verifier_mask & (pd.notna(out["verifier_keep_error_prob"])) & (
        safe_num_series(out, "verifier_keep_error_prob", 0.0) <
        safe_num_series(out, "group_verifier_threshold", verifier_threshold)
    )
    final_pred.loc[reject_mask] = 0

    out["final_pred_label"] = final_pred.astype(int)
    out["final_pred_label_name"] = out["final_pred_label"].map({1: "error", 0: "correct"})
    return out
...
def apply_column_post_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mask1 = (
        out["final_pred_label"].astype(int).eq(1) &
...
def mark_hard_cases(df: pd.DataFrame, detector_thresholds: pd.Series) -> pd.DataFrame:
    ...
    out["sample_borderline_hardcase"] = (
        ...
        (
            (margin <= 0.08) |
            (pd.notna(verifier_prob) & verifier_prob.between(0.40, 0.60))
        )
    ).astype(int)
    ...
    out["recall_recovery_hardcase"] = (
        ...
        final_label.eq(0) &
        (
            ((detector_thresholds - detector_prob) >= 0.0) & ((detector_thresholds - detector_prob) <= 0.10)
        )
    ).astype(int)
    ...
    out["is_hard_case"] = (
        ...
        ((out["is_near_threshold"] == 1) & (out["is_stage_disagree"] == 1))
    ).astype(int)
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for _, row in df.iterrows():
            rec = {
                ...
                "detector_pred_error_prob": float(row.get("detector_pred_error_prob", 0.0)),
                "final_pred_error_prob": float(row.get("final_pred_error_prob", 0.0)),

                "raw_pred_label": int(row.get("raw_pred_label", 0)),
                "detector_pred_label": int(row.get("detector_pred_label", 0)),
                "final_pred_label": int(row.get("final_pred_label", 0)),

                "verifier_keep_error_prob": None if pd.isna(row.get("verifier_keep_error_prob", np.nan))
                else float(row.get("verifier_keep_error_prob", 0.0)),
                "verifier_decision": row.get("verifier_decision", None),
                "group_verifier_threshold": None if pd.isna(row.get("group_verifier_threshold", np.nan))
                else float(row.get("group_verifier_threshold", 0.50)),
                ...
                "hard_case_flags": {
                    ...
                    "verifier_borderline": int(row.get("verifier_borderline", 0)),
                    "verifier_reject_borderline": int(row.get("verifier_reject_borderline", 0)),
                    "fp_conflict_fd_hardcase": int(row.get("fp_conflict_fd_hardcase", 0)),
                    "sample_borderline_hardcase": int(row.get("sample_borderline_hardcase", 0)),
                    "recall_recovery_hardcase": int(row.get("recall_recovery_hardcase", 0)),
...
def main():
    ...
    if PRINT_VERIFIER_DEBUG:
        print(f"[DEBUG] verifier threshold mode = {VERIFIER_THRESHOLD_MODE}")
        print(f"[DEBUG] verifier threshold source = {threshold_source}")
        print(f"[DEBUG] verifier threshold used = {verifier_threshold:.4f}")
    ...
    out_df["stage1_mid_zone"] = (
        (out_df["raw_pred_error_prob"] >= HARD_CASE_LOW) &
        (out_df["raw_pred_error_prob"] <= HARD_CASE_HIGH)
    ).astype(int)

    if USE_STAGE2:
        X2 = build_feature_matrix(out_df, stage2_cfg["feature_cols"], stage2_preprocessor)
        detector_prob = predict_probs(stage2_model, X2)
    else:
        detector_prob = raw_prob

    out_df["detector_pred_error_prob_before_adjust"] = detector_prob
    out_df["detector_pred_error_prob"] = apply_neighbor_adjustment(out_df, "detector_pred_error_prob_before_adjust")

    detector_thresholds = apply_column_thresholds(out_df, stage2_threshold)
    out_df["detector_decision_threshold"] = detector_thresholds
    out_df["detector_pred_label"] = (out_df["detector_pred_error_prob"] >= out_df["detector_decision_threshold"]).astype(int)
    out_df["detector_pred_label_name"] = out_df["detector_pred_label"].map({1: "error", 0: "correct"})
    out_df["detector_margin_to_threshold"] = out_df["detector_pred_error_prob"] - out_df["detector_decision_threshold"]
    out_df["is_stage_disagree"] = (out_df["raw_pred_label"] != out_df["detector_pred_label"]).astype(int)

    out_df["final_pred_error_prob"] = out_df["detector_pred_error_prob"]
    out_df["final_decision_threshold"] = out_df["detector_decision_threshold"]
    ...
    out_df["is_high_risk"] = (
        (out_df["final_pred_error_prob"] >= HIGH_RISK_THRESHOLD) &
        (out_df["final_pred_label"] == 1)
    ).astype(int)
