...
OUTPUT_DIR = "two_stage_model_output_v2"
...
CAND_RULE_SCALE = 0.15
FD_LIKE_SCALE = 0.25
CTX_RULE_SCALE = 0.20
GLOBAL_RULE_SCALE = 0.10
TYPO_RULE_SCALE = 0.30
PATTERN_RULE_SCALE = 0.15
SCHEMA_RULE_SCALE = 0.20
RARE_VALUE_RULE_SCALE = 0.10
...
# 说明：
# 1）去掉 label_source，避免训练-推理不一致
# 2）is_outside_candidate 默认不作为模型输入，只保留在数据中
# 3）新增 rare/typo/pattern/schema 计数特征
...
def add_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["rule_total_count"] = (
        out["strong_rule_count"]
        + out["candidate_generation_rule_count"]
    ).astype(float)
    ...
    out["rule_strength_sum"] = (
        ...
        0.3 * out["candidate_generation_rule_count"] +
        0.4 * out["fd_like_count"] +
        0.3 * out["context_rule_count"] +
        0.2 * out["global_rule_count"] +
        0.5 * out["typo_rule_count"] +
        0.2 * out["pattern_rule_count"] +
...
class TabularDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for k, v in meta.items():
            self.meta[k] = torch.tensor(v, dtype=torch.float32)
...
def build_neighbor_soft_target(
    y_true: torch.Tensor,
    is_equal_to_neighbor_majority: torch.Tensor,
    neighbor_majority_ratio: torch.Tensor,
):
    ratio = torch.clamp(neighbor_majority_ratio, 0.0, 1.0)
    valid_mask = (ratio >= MIN_NEIGHBOR_RATIO_FOR_LOSS).float()

    base = torch.where(
        is_equal_to_neighbor_majority > 0.5,
        1.0 - ratio,
        ratio
    )
...
def build_rule_soft_target(
    strong_rule_count: torch.Tensor,
    candidate_generation_rule_count: torch.Tensor,
    fd_like_count: torch.Tensor,
    context_rule_count: torch.Tensor,
    global_rule_count: torch.Tensor,
    rare_value_count: torch.Tensor,
    typo_rule_count: torch.Tensor,
    pattern_rule_count: torch.Tensor,
    schema_rule_count: torch.Tensor,
):
    score = (
...
def composite_loss(
    ...
    # neighbor
    neighbor_soft_target, neighbor_valid_mask = build_neighbor_soft_target(
        y_true=targets,
        is_equal_to_neighbor_majority=batch["is_equal_to_neighbor_majority"],
        neighbor_majority_ratio=batch["neighbor_majority_ratio"],
    )

    neighbor_bce = nn.functional.binary_cross_entropy(
        probs,
        neighbor_soft_target,
        reduction="none"
    )
    neighbor_weight = sample_weights * neighbor_valid_mask
    if torch.sum(neighbor_weight) > 0:
        neighbor_loss = (neighbor_bce * neighbor_weight).sum() / (neighbor_weight.sum() + 1e-8)
    else:
        neighbor_loss = torch.tensor(0.0, device=logits.device)

    # rule
    rule_soft_target = build_rule_soft_target(
        strong_rule_count=batch["strong_rule_count"],
        candidate_generation_rule_count=batch["candidate_generation_rule_count"],
        fd_like_count=batch["fd_like_count"],
        context_rule_count=batch["context_rule_count"],
        global_rule_count=batch["global_rule_count"],
        rare_value_count=batch["rare_value_count"],
        typo_rule_count=batch["typo_rule_count"],
        pattern_rule_count=batch["pattern_rule_count"],
        schema_rule_count=batch["schema_rule_count"],
    )

    rule_bce = nn.functional.binary_cross_entropy(
        probs,
        rule_soft_target,
        reduction="none"
    )
    rule_weight = sample_weights
    if torch.sum(rule_weight) > 0:
        rule_loss = (rule_bce * rule_weight).sum() / (rule_weight.sum() + 1e-8)
    else:
        rule_loss = torch.tensor(0.0, device=logits.device)

    total_loss = cls_loss + LAMBDA_NEIGHBOR * neighbor_loss + LAMBDA_RULE * rule_loss

    loss_detail = {
        "total_loss": float(total_loss.detach().cpu().item()),
        "cls_loss": float(cls_loss.detach().cpu().item()),
        "neighbor_loss": float(neighbor_loss.detach().cpu().item()),
        "rule_loss": float(rule_loss.detach().cpu().item()),
    }
...
def run_stage_training(
    ...
    if USE_POS_WEIGHT:
        pos_weight_value = build_pos_weight(y_train)
    else:
...
def main():
    ...
    summary = {
        ...
        "loss_design": {
            ...
            "min_neighbor_ratio_for_loss": MIN_NEIGHBOR_RATIO_FOR_LOSS,
            "strong_rule_scale": STRONG_RULE_SCALE,
            "candidate_generation_rule_scale": CAND_RULE_SCALE,
            "fd_like_scale": FD_LIKE_SCALE,
            "context_rule_scale": CTX_RULE_SCALE,
            "global_rule_scale": GLOBAL_RULE_SCALE,
            "typo_rule_scale": TYPO_RULE_SCALE,
            "pattern_rule_scale": PATTERN_RULE_SCALE,
            "schema_rule_scale": SCHEMA_RULE_SCALE,
            "rare_value_rule_scale": RARE_VALUE_RULE_SCALE,
        },
