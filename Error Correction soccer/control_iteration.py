...
# -*- coding: utf-8 -*-
"""
Soccer iterative repair controller.

Complete Soccer version adapted from Adult control_iteration.py.

Pipeline:
1. Bootstrap accumulated pairwise labels.
2. Build inference whitelist feature file.
3. Train Soccer pairwise LTR student.
4. Run Soccer inference + conservative gate.
5. Send hard cases to LLM for pairwise ranking.
6. Merge new labels and iterate.

Default files:
  repair_candidate_features_v2.csv
  predicted_error_positions_soccer_augmented.csv
  repair_pairwise_responses_v2.jsonl
  build_infer_features_whitelist_soccer.py
  train_ltr.py
  infer_ltr.py
...
def parse_args():
    p = argparse.ArgumentParser(description="Soccer iterative repair controller")
    ...
    p.add_argument("--work_dir", default="repair_iterative_workdir_soccer")
...
def build_column_specific_instruction(column: str) -> str:
    ...
    return "优先考虑规则一致性、Soccer profile 一致性、domain 合法性和候选来源可靠性。"
...
2. position 必须是合法位置或明确规范化。
3. birthyear/season 必须是合法年份，并且年龄关系合理。
4. team/city/stadium/manager 需要上下文一致性，不能只凭高频。
5. name/surname/birthplace 不能凭高频猜测；profile_key_majority/entity_pair_majority/candidate_entity_safe_fill 是比 column_top/global_dictionary 更强的证据。
6. position 中 team_season_position/candidate_position_profile_strong 应优先于单纯 domain_position_core_value。
7. 如果两个候选都不可靠，输出 Unknown。
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt == MAX_RETRIES:
                return {"winner": "Unknown", "reason_type": "uncertain", "reason": f"LLM call failed: {str(e)}"}
            time.sleep(RETRY_SLEEP_SECONDS)


def label_hard_cases_with_llm():
    if int(ARGS.skip_llm_relabel) == 1:
    ...
    client = get_client()
    new_rows = []
    limit = min(ARGS.max_hard_cases_per_iter, ARGS.llm_max_per_iter, len(hard_cases))

    for i, obj in enumerate(hard_cases[:limit], start=1):
        ...
        new_rows.append({
            ...
            "source": "iterative_hard_case_llm_soccer",
        })
        if i % 20 == 0:
            print(f"[INFO] LLM hard-case progress: {i}/{limit}")
...
def main():
    ...
    print("\n========== Soccer iterative repair finished ==========")
