"""
Rayyan pairwise LTR training script, logic-migrated from the Flights training pipeline.

Migration principle:
- Keep the full Flights training framework:
  pairwise materialization, shared scorer MLP, reason auxiliary head,
  rule regularization, column profiles, resume checkpoint, early stopping,
  feature alignment JSON, and training logs.
- Convert Flights-specific weighting / derived features:
  ...
  time-source boost -> Rayyan metadata/canonical/consensus source boost.
"""
...
def parse_args():
    ...
    p.add_argument("--output_dir", default="pairwise_ltr_student_output_rayyan")
...
def add_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Rayyan canonical / metadata 派生特征。
    # 保留 is_time_column_derived 等旧字段名以兼容原 Flights 框架，但语义迁移为 Rayyan 特殊列。
    for col_name in [
    ...
    out["dirty_language_canonical_derived"] = [normalize_language_candidate(x) or "" for x in out["dirty_value"]]
    out["candidate_language_canonical_derived"] = [normalize_language_candidate(x) or "" for x in out["candidate_value"]]
    out["dirty_issn_canonical_derived"] = [normalize_issn_candidate(x) or "" for x in out["dirty_value"]]
    out["candidate_issn_canonical_derived"] = [normalize_issn_candidate(x) or "" for x in out["candidate_value"]]
    out["dirty_date_canonical_derived"] = [normalize_date_candidate(x) or "" for x in out["dirty_value"]]
    out["candidate_date_canonical_derived"] = [normalize_date_candidate(x) or "" for x in out["candidate_value"]]
...
def build_candidate_index(df: pd.DataFrame, feature_columns: List[str]):
    ...
    for _, row in df.iterrows():
        ...
        # Rayyan consensus 版特征：metadata consensus / canonical restore 候选应该在 pairwise 学习中更有权重。
        is_consensus = max(
            safe_float(row.get("is_from_rayyan_consensus", 0.0), 0.0),
            safe_float(row.get("contains_rayyan_consensus_source", 0.0), 0.0),
            safe_float(row.get("rayyan_consensus_candidate", 0.0), 0.0),
            safe_float(row.get("candidate_and_allowed_both_rayyan_consensus", 0.0), 0.0),
        )
        consensus_conf = max(
            safe_float(row.get("rayyan_consensus_table_max_confidence", 0.0), 0.0),
            safe_float(row.get("allowed_consensus_confidence", 0.0), 0.0),
            safe_float(row.get("rayyan_consensus_confidence_from_info", 0.0), 0.0),
        )
        consensus_sources = max(
            safe_float(row.get("rayyan_consensus_table_max_support_count", 0.0), 0.0),
            safe_float(row.get("allowed_consensus_support_count", 0.0), 0.0),
            safe_float(row.get("rayyan_consensus_support_count_from_info", 0.0), 0.0),
        )

        if is_consensus > 0:
            boost = 1.0 + 0.7 * min(max(consensus_conf, 0.0), 1.0) + 0.04 * min(consensus_sources, 10.0)
            boost = min(boost, 2.2)
            sample_weight *= boost

        # language / ISSN / date / volume / issue / pagination 的 canonical restore 信号适当加权。
        src = str(row.get("candidate_source", row.get("candidate_source_text", ""))).lower()
        if (
            "canonical_language_restore" in src
            or "canonical_issn_restore" in src
            or "canonical_date_restore" in src
            or "canonical_volume_restore" in src
            or "canonical_issue_restore" in src
            or "canonical_pagination_restore" in src
            or "rayyan_metadata_consensus" in src
        ):
            sample_weight *= 1.25

        if safe_float(row.get("rayyan_candidate_is_valid_issn", 0.0), 0.0) > 0:
            sample_weight *= 1.10
