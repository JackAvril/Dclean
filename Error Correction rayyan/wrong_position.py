...
INPUT_FILE = "/mnt/mydata/dq/projects/Splittree/test/Error Detection rayyan/active_cleaning_loop_runs_rayyan_v4/iter_3/infer/inference_all_predictions.csv"
...
def pick_error_mask(df: pd.DataFrame):
    ...
    if "final_pred_label_name" in df.columns:
        ...
        return df["final_pred_label_name"].astype(str).str.strip().str.lower() == "error"
    ...
    if "detector_pred_label_name" in df.columns:
        ...
        return df["detector_pred_label_name"].astype(str).str.strip().str.lower() == "error"
    ...
    if "raw_pred_label_name" in df.columns:
        ...
        return df["raw_pred_label_name"].astype(str).str.strip().str.lower() == "error"
...
if "canonical_bucket" in result_df.columns:
    print("\n---- canonical_bucket 分布 ----")
    print(result_df["canonical_bucket"].value_counts(dropna=False).head(20).to_string())

if "date_value_bucket" in result_df.columns:
    print("\n---- date_value_bucket 分布 ----")
    print(result_df["date_value_bucket"].value_counts(dropna=False).head(20).to_string())

if "language_bucket" in result_df.columns:
    print("\n---- language_bucket 分布 ----")
    print(result_df["language_bucket"].value_counts(dropna=False).head(20).to_string())
