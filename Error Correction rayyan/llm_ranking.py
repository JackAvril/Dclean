"""
Rayyan LLM sampling + pairwise labeling script, logic-migrated from the Flights pipeline.

Migration principle:
- Keep the full Flights framework:
  cell-level summary, bucket assignment, KMeans clustering, center/boundary/outlier sampling,
  candidate export, pairwise request generation, one_vs_top/all_pairs strategies,
  parallel LLM calls, resume from existing responses, and teacher ranking aggregation.
- Convert Flights-specific logic:
  time/flight consensus -> Rayyan journal/article metadata consensus
  time canonicalization -> language / ISSN / date / volume / issue / pagination canonicalization
  time-source features -> Rayyan canonical / metadata / consensus source features
  time-specific prompt -> Rayyan column-specific prompt
- Preserve downstream outputs:
...
INPUT_CANDIDATE_FEATURES_CSV = "/mnt/mydata/dq/projects/Splittree/test/Error Correction rayyan/repair_candidate_features_infer_whitelist.csv"


# 输入2：详细上下文 rich jsonl
INPUT_DETAIL_JSONL = "/mnt/mydata/dq/projects/Splittree/test/Error Detection rayyan/candidate_llm_contexts_rayyan.jsonl"
...
# 是否把 column 纳入分桶
# Rayyan 建议 True，因为 article_language / journal_issn / date / volume / issue 等列的修复语义不同。
...
# Rayyan hard columns
# 迁移 Flights 的 hard-column 采样思想：
# Flights 中时间列更难；Rayyan 中 language / ISSN / date / volume / issue / pagination / journal metadata 更难。
RAYYAN_HARD_COLUMNS = {
    "article_language",
    "journal_issn",
    "journal_title",
...
TIME_COLUMNS = set()
HARD_COLUMNS = RAYYAN_HARD_COLUMNS
...
def looks_like_rayyan_canonical_column(column: str, semantic_type: Optional[str] = None) -> bool:
    col = norm_text(column)
    sem = norm_text(semantic_type)
    return (
        sem in {"date_like", "language_like", "issn_like", "canonical_like"}
        or col in {
            "article_language",
            "journal_issn",
            "article_jcreated_at",
...
def looks_like_time_column(column: str, semantic_type: Optional[str] = None) -> bool:
    # 兼容原函数名，供旧框架调用；Rayyan 中实际含义为 canonical/metadata-sensitive column。
    return looks_like_rayyan_canonical_column(column, semantic_type)
...
def build_cell_summary_df(cand_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (row_id, column), g in grouped:
        ...
        # Rayyan canonical/metadata features
        is_time_like_column = int(
            looks_like_time_column(str(column), semantic_type)
            or (get_numeric_series(g, "rayyan_column_is_article_language", 0.0).max() > 0)
            or (get_numeric_series(g, "rayyan_column_is_journal_issn", 0.0).max() > 0)
            or (get_numeric_series(g, "rayyan_column_is_article_jcreated_at", 0.0).max() > 0)
            or (get_numeric_series(g, "rayyan_column_is_article_jvolumn", 0.0).max() > 0)
            or (get_numeric_series(g, "rayyan_column_is_article_jissue", 0.0).max() > 0)
            or (get_numeric_series(g, "rayyan_column_is_article_pagination", 0.0).max() > 0)
        )

        rayyan_metadata_source_any = get_numeric_series(g, "contains_rayyan_metadata_source", 0.0).max()
        rayyan_journal_metadata_source_any = get_numeric_series(g, "contains_journal_metadata_source", 0.0).max()
        rayyan_article_metadata_source_any = get_numeric_series(g, "contains_article_metadata_source", 0.0).max()
        rayyan_canonical_source_any = get_numeric_series(g, "contains_canonical_source", 0.0).max()
        rayyan_consensus_source_any = get_numeric_series(g, "contains_rayyan_consensus_source", 0.0).max()

        rayyan_consensus_confidence_max = get_numeric_series(g, "rayyan_consensus_table_max_confidence", 0.0).max()
        rayyan_consensus_margin_max = get_numeric_series(g, "rayyan_consensus_table_max_margin", 0.0).max()
        rayyan_consensus_support_max = get_numeric_series(g, "rayyan_consensus_table_max_support_count", 0.0).max()
        rayyan_consensus_table_match_max = get_numeric_series(g, "rayyan_consensus_table_match", 0.0).max()
        rayyan_high_conf_consensus_match_max = get_numeric_series(g, "rayyan_consensus_table_high_conf_match", 0.0).max()

        rayyan_valid_language_max = get_numeric_series(g, "rayyan_candidate_is_language", 0.0).max()
        rayyan_valid_issn_max = get_numeric_series(g, "rayyan_candidate_is_issn", 0.0).max()
        rayyan_valid_issn_checksum_max = get_numeric_series(g, "rayyan_candidate_is_valid_issn", 0.0).max()
        rayyan_valid_date_max = get_numeric_series(g, "rayyan_candidate_is_date", 0.0).max()
        rayyan_valid_volume_max = get_numeric_series(g, "rayyan_candidate_is_volume", 0.0).max()
        rayyan_valid_issue_max = get_numeric_series(g, "rayyan_candidate_is_issue", 0.0).max()
        rayyan_valid_pagination_max = get_numeric_series(g, "rayyan_candidate_is_pagination", 0.0).max()
        rayyan_format_restore_any = get_numeric_series(g, "rayyan_candidate_is_format_restoration", 0.0).max()

        top1_rayyan_metadata_source = safe_int(g["contains_rayyan_metadata_source"].iloc[0], 0) if "contains_rayyan_metadata_source" in g.columns and len(g) >= 1 else 0
        top1_rayyan_canonical_source = safe_int(g["contains_canonical_source"].iloc[0], 0) if "contains_canonical_source" in g.columns and len(g) >= 1 else 0
        top1_rayyan_consensus_source = safe_int(g["contains_rayyan_consensus_source"].iloc[0], 0) if "contains_rayyan_consensus_source" in g.columns and len(g) >= 1 else 0

        # Keep legacy names for downstream compatibility; values now reflect Rayyan canonical confidence.
        candidate_is_valid_time_max = max(
            rayyan_valid_language_max,
            rayyan_valid_issn_max,
            rayyan_valid_date_max,
            rayyan_valid_volume_max,
            rayyan_valid_issue_max,
            rayyan_valid_pagination_max,
        )
        avg_time_context_gain = get_numeric_series(g, "rayyan_consensus_table_max_confidence", 0.0).mean()
        max_time_context_gain = rayyan_consensus_confidence_max
        min_time_diff = 0.0
        time_source_any = int(rayyan_metadata_source_any or rayyan_canonical_source_any or rayyan_consensus_source_any)
        top1_time_context_gain = rayyan_consensus_confidence_max
        top1_candidate_is_valid_time = int(candidate_is_valid_time_max)
        top1_candidate_in_time_range = int(rayyan_consensus_table_match_max or rayyan_format_restore_any)
        ...
        rows.append({
            ...
            "rayyan_metadata_source_any": int(rayyan_metadata_source_any),
            "rayyan_journal_metadata_source_any": int(rayyan_journal_metadata_source_any),
            "rayyan_article_metadata_source_any": int(rayyan_article_metadata_source_any),
            "rayyan_canonical_source_any": int(rayyan_canonical_source_any),
            "rayyan_consensus_source_any": int(rayyan_consensus_source_any),
            "rayyan_consensus_confidence_max": float(rayyan_consensus_confidence_max),
            "rayyan_consensus_margin_max": float(rayyan_consensus_margin_max),
            "rayyan_consensus_support_max": float(rayyan_consensus_support_max),
            "rayyan_consensus_table_match_max": int(rayyan_consensus_table_match_max),
            "rayyan_high_conf_consensus_match_max": int(rayyan_high_conf_consensus_match_max),
            "rayyan_valid_language_max": int(rayyan_valid_language_max),
            "rayyan_valid_issn_max": int(rayyan_valid_issn_max),
            "rayyan_valid_issn_checksum_max": int(rayyan_valid_issn_checksum_max),
            "rayyan_valid_date_max": int(rayyan_valid_date_max),
            "rayyan_valid_volume_max": int(rayyan_valid_volume_max),
            "rayyan_valid_issue_max": int(rayyan_valid_issue_max),
            "rayyan_valid_pagination_max": int(rayyan_valid_pagination_max),
            "rayyan_format_restore_any": int(rayyan_format_restore_any),
            "top1_rayyan_metadata_source": int(top1_rayyan_metadata_source),
            "top1_rayyan_canonical_source": int(top1_rayyan_canonical_source),
            "top1_rayyan_consensus_source": int(top1_rayyan_consensus_source),
...
def get_bucket_keys() -> List[str]:
    keys = [
        ...
        "is_hard_column",
        "is_time_like_column",
        "rayyan_metadata_source_any",
        "rayyan_canonical_source_any",
        "rayyan_consensus_source_any",
    ]
...
FEATURE_COLUMNS_NUMERIC = [
    ...
    # Rayyan migrated metadata/canonical/consensus features
    "rayyan_metadata_source_any",
    "rayyan_journal_metadata_source_any",
    "rayyan_article_metadata_source_any",
    "rayyan_canonical_source_any",
    "rayyan_consensus_source_any",
    "rayyan_consensus_confidence_max",
    "rayyan_consensus_margin_max",
    "rayyan_consensus_support_max",
    "rayyan_consensus_table_match_max",
    "rayyan_high_conf_consensus_match_max",
    "rayyan_valid_language_max",
    "rayyan_valid_issn_max",
    "rayyan_valid_issn_checksum_max",
    "rayyan_valid_date_max",
    "rayyan_valid_volume_max",
    "rayyan_valid_issue_max",
    "rayyan_valid_pagination_max",
    "rayyan_format_restore_any",
    "top1_rayyan_metadata_source",
    "top1_rayyan_canonical_source",
    "top1_rayyan_consensus_source",
...
CANDIDATE_FEATURE_EXPORT_COLS = [
    ...
    # Rayyan metadata/canonical/consensus-style
    ...
    "rayyan_consensus_info",
    "rayyan_consensus_confidence_from_info",
    "rayyan_consensus_margin_from_info",
    "rayyan_consensus_support_count_from_info",
    ...
    "rayyan_consensus_table_match_count",
    "rayyan_candidate_is_language",
    "rayyan_dirty_is_language",
    "rayyan_candidate_language_equals_row_language",
    "rayyan_candidate_language_restoration",
    "rayyan_candidate_is_issn",
    "rayyan_dirty_is_issn",
    "rayyan_candidate_is_valid_issn",
    "rayyan_dirty_is_valid_issn",
    "rayyan_candidate_issn_equals_row_issn",
    "rayyan_candidate_issn_restoration",
    "rayyan_candidate_is_date",
    "rayyan_dirty_is_date",
    "rayyan_candidate_date_equals_row_date",
    "rayyan_candidate_date_restoration",
    "rayyan_candidate_is_volume",
    "rayyan_dirty_is_volume",
    "rayyan_candidate_volume_equals_row_volume",
    "rayyan_candidate_is_issue",
    "rayyan_dirty_is_issue",
    "rayyan_candidate_issue_equals_row_issue",
    "rayyan_candidate_is_pagination",
    "rayyan_dirty_is_pagination",
    "rayyan_candidate_pagination_restoration",
    "rayyan_candidate_is_format_restoration",
    "rayyan_candidate_normalized_equals_dirty",
    "rayyan_is_language_candidate",
    "rayyan_is_issn_candidate",
    "rayyan_is_valid_issn_candidate",
    "rayyan_is_date_candidate",
    "rayyan_is_volume_candidate",
    "rayyan_is_issue_candidate",
    "rayyan_is_pagination_candidate",
    ...
    "allowed_date_value",
    "allowed_language_canonical",
    "allowed_issn_canonical",
    "allowed_canonical_bucket",
    "allowed_date_value_bucket",
    "allowed_language_bucket",
    ...
    # Flights consensus-style
    "is_from_flight_consensus",
    "contains_flight_consensus_source",
    "contains_trusted_flight_consensus_source",
    "candidate_equals_flight_consensus",
    "flight_consensus_confidence",
    "flight_consensus_margin",
    "flight_consensus_support_count",
    "flight_consensus_source_count",
    "allowed_consensus_value",
    "allowed_consensus_confidence",
    "allowed_consensus_margin",
    "allowed_consensus_support_count",
    "allowed_consensus_source_count",
    "allowed_time_diff_to_consensus",

    # Flights time-style
    "contains_time_source",
    "contains_time_dirty_extract_source",
    "contains_time_pattern_restore_source",
    "contains_time_column_dictionary_source",
    "contains_time_row_context_source",
        ...
        for _, row in sampled_df.iterrows():
            ...
            out = {
                ...
                "cell_summary": {
                    ...
                    "rayyan_metadata_source_any": row.get("rayyan_metadata_source_any"),
                    "rayyan_canonical_source_any": row.get("rayyan_canonical_source_any"),
                    "rayyan_consensus_source_any": row.get("rayyan_consensus_source_any"),
                    "rayyan_consensus_confidence_max": row.get("rayyan_consensus_confidence_max"),
                    "rayyan_consensus_support_max": row.get("rayyan_consensus_support_max"),
                    "rayyan_format_restore_any": row.get("rayyan_format_restore_any"),
...
def build_context_for_prompt(detail_context: Dict[str, Any], dirty_value: str, column: str) -> Dict[str, Any]:
    ...
    if isinstance(row_ctx, dict):
        ...
        # Rayyan rich metadata bundle
        row_values_safe = row_ctx.get("row_values", {})
        if isinstance(row_values_safe, dict):
            out["rayyan_metadata_bundle"] = {
                "id": row_values_safe.get("id"),
                "article_title": row_values_safe.get("article_title"),
                "article_language": row_values_safe.get("article_language"),
                "journal_title": row_values_safe.get("journal_title"),
                "jounral_abbreviation": row_values_safe.get("jounral_abbreviation", row_values_safe.get("journal_abbreviation")),
                "journal_issn": row_values_safe.get("journal_issn"),
                "article_jvolumn": row_values_safe.get("article_jvolumn"),
                "article_jissue": row_values_safe.get("article_jissue"),
                "article_jcreated_at": row_values_safe.get("article_jcreated_at"),
                "article_pagination": row_values_safe.get("article_pagination"),
            }
    ...
    if isinstance(col_ctx, dict):
        out["column_context"] = {
            ...
            "date_stats": col_ctx.get("date_stats"),
            "date_value": col_ctx.get("date_value"),
    ...
    if isinstance(conf_ctx, dict):
        ...
        out["conflict_context"] = {
            ...
            "canonical_rule_count": conf_ctx.get("canonical_rule_count"),
...
def build_column_specific_instruction(column: str) -> str:
    ...
    if column == "article_language":
        return (
            "本列是文章语言列。优先选择 ISO-like 三字母规范值，例如 eng/fre/ger/spa；"
            "重点参考 journal_title、jounral_abbreviation、journal_issn、article_title 对 article_language 的一致性；"
            "如果候选来自 rayyan_metadata_consensus、journal_title_to_language、issn_to_language 或 canonical_language_restore，"
            "且规则支持、neighbor 支持或 consensus confidence 较高，应优先考虑。"
        )
    ...
    if column == "journal_issn":
        return (
            ...
            "候选如果通过 ISSN 校验位更可信；"
            "重点参考 journal_title 与 jounral_abbreviation 对 ISSN 的一致性，"
            "候选来自 rayyan_metadata_consensus、journal_title_to_issn、journal_abbr_to_issn、canonical_issn_restore 时要重点考虑。"
        )

    if column == "journal_title":
        return (
            "本列是期刊全名。优先选择与 journal_issn、jounral_abbreviation、article_language 一致的期刊名；"
            "不要只因为列内高频或字符串相似选择候选。"
        )
    ...
    if column in {"jounral_abbreviation", "journal_abbreviation"}:
        return (
            ...
            "注意原字段名可能拼写为 jounral_abbreviation。"
        )
    ...
    if column == "article_jcreated_at":
        return (
            ...
            "不要把本来合法的日期无根据改成列内高频日期。"
        )

    if column == "article_jvolumn":
        return (
            "本列是期刊卷号。优先选择合法整数卷号，并参考 journal_title + article_jissue 对 article_jvolumn 的共识；"
            "候选来自 journal_issue_to_volume 或 canonical_volume_restore 时要重点考虑。"
        )

    if column == "article_jissue":
        return (
            "本列是期刊期号。优先选择合法整数期号，并参考 journal_title + article_jvolumn 对 article_jissue 的共识；"
            "候选来自 journal_volume_to_issue 或 canonical_issue_restore 时要重点考虑。"
        )
    ...
    if column == "article_title":
        return (
            "本列是文章标题。优先选择与 id、journal metadata、语言上下文一致的标题；"
            "不要只根据局部字符串相似或列内高频选择候选。"
        )

    return (
        "优先考虑规则一致性、同行上下文一致性、metadata consensus、统计支持和合理的拼写/规范化变换。"
    )
...
通用判断原则：
1. 优先考虑规则一致性：FD、soft FD、上下文主导值、metadata/canonical 约束等是否支持该候选
2. 再考虑同行上下文一致性：该候选是否与同一行其它属性更匹配
3. 再考虑统计支持：列内高频、共现概率、邻居支持、similar row 支持
4. 再考虑变换合理性：拼写相似、键盘误触、发音相似、language/ISSN/date/pagination 格式恢复
5. 如果两个候选都明显不合理，输出 Unknown
6. 不要仅因为候选更常见就无条件选择它
7. 对 Rayyan 特殊列，优先选择规则强支持、metadata consensus 强支持且符合列语义规范的候选
...
def main():
    ...
    for bucket_id, bucket_df in cell_summary_df.groupby("bucket_id", sort=False):
        ...
        for cluster_id, cluster_df in bucket_clustered.groupby("cluster_id", sort=False):
            ...
            bucket_summary_rows.append({
                ...
                "rayyan_metadata_source_any": int(cluster_df["rayyan_metadata_source_any"].iloc[0]) if "rayyan_metadata_source_any" in cluster_df.columns else 0,
                "rayyan_canonical_source_any": int(cluster_df["rayyan_canonical_source_any"].iloc[0]) if "rayyan_canonical_source_any" in cluster_df.columns else 0,
                "rayyan_consensus_source_any": int(cluster_df["rayyan_consensus_source_any"].iloc[0]) if "rayyan_consensus_source_any" in cluster_df.columns else 0,
            })
    ...
    if len(sampled_df) > 0:
        ...
        if "is_time_like_column" in sampled_df.columns:
            print("\nRayyan canonical/metadata 列采样分布:")
