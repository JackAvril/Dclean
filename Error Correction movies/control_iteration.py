...
def parse_args():
    ...
    p.add_argument("--work_dir", default="repair_iterative_workdir_movies")
...
def build_column_specific_instruction(column: str) -> str:
    ...
    if column == "Duration":
        return "本列是电影时长列。优先选择合法的时长格式，例如 90 min；结合 Name/Year/Director/Genre/Rating 以及 movie_metadata_consistency 判断，不要仅因 90 min 高频就选择。"
    if column == "Year":
        return "本列是电影年份。优先选择 1800-2100 范围内四位年份；重点看 Release Date 年份、Id/Name 对 Year 的一致性和 expected value。"
    if column == "Release Date":
        return "本列是上映日期。检查日期格式、年份是否与 Year 一致、括号国家是否与 Country 一致。"
    if column in {"RatingValue", "RatingCount", "ReviewCount"}:
        return "本列是评分/计数字段。优先选择合法范围/非负整数/规范评论格式，并结合 Id、Name+Year、Rating 上下文。"
    if column in {"Country", "Language"}:
        return "本列是国家/语言。高频值容易误修，必须有规则、上下文或 metadata consistency 支持。"
    if column in {"Actors", "Cast", "Director", "Creator", "Genre", "Filming Locations", "Name"}:
        return "本列是文本/列表元数据。除非有强规则、Id/Name-Year 或 movie metadata consistency 支持，否则不要轻易用列内高频候选替换。"
    return "优先考虑规则一致性、同行电影元数据一致性、列类型合法性、统计支持和合理的格式恢复。"
...
2. 再考虑同行电影元数据一致性：Name、Year、Release Date、Country、Duration、Rating、Actors/Cast 是否匹配
3. 再考虑列类型合法性：年份、日期、时长、评分、计数、人物列表、国家/语言格式是否合理
4. 再考虑统计支持和字符串/格式变换合理性
